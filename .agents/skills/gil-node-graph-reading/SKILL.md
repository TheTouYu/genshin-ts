...
### Step 3.5 修复后读图自检（2026-08-14 方法论强制，勿跳；2026-08-30 升级逐项勾对）
**修改生产代码（编译/注入后）的第一件事是读图自检，通过后才交用户游戏测试。**
编译产物 .gia 正确 ≠ 注入后 .gil 正确（#20b/#20c 实证：.gia 有边、.gil 丢边——
注入器按 CompositeDef 接口裁剪调用点引脚）。自检清单：

> **2026-08-30 拒载事故教训（勿再犯）**：本清单每条都是"必跑项"，不是"跑几个检查"。
> 实测案例：注入后只跑 `check-gil-composite-refs`（不带 --incoming）看到"✓ 0 悬空"就交付，
> 漏跑 --incoming → 旧冲量踢球残留链（auto_check_tick→dribble_decide）类型错位漏检 →
> **游戏拒载无日志**（足球 1073741908，SHA 0ae6f5d1）。回测证明 --incoming 完全能抓住
> 该残留链（3 条"类型错位"）。**"✓ 0 悬空"≠"游戏能加载"：0 悬空只证明引用存在，
> 不证明地图里没有多余的残留 def 类型错位。**
>
> 逐项勾对法：把下列每一条当作 checklist 逐项执行，每项记录命令输出关键行，
> 不允许"跑过工具就算过"——必须核对输出含义（0 悬空？缺失误报？类型错位？）。
...
- **① 全量复合引用完整性（2026-08-20 注入事故后必跑）**：`npx tsx tools/check-gil-composite-refs.ts <地图.gil> --incoming <本次.gia>`——
  ...
  - **输出判读**：`✗ 残留复合 X 引用的 ID ... 被本次注入覆盖——可能类型错位` = **真问题，必须清理残留后重注入**
    （见 gil-node-graph-editing 技能 def-clean 残留链清理）；`✓ 0 悬空` 只代表引用存在，**不代表无残留**。
  ...
  **信号定义单元**也收进对比，会报「GIA 复合 16106127xx 注入后在地图中缺失」——这是误报，信号单元以信号形态注册
  ...
  - 另一类误报（2026-08-30 实证）：本次 GIA 未注入的旧 def 段（如 2000000000 段 motion_by_vel 等）
    报「注入后在地图中缺失」——那是旧版本注入的 def，本次未覆盖，非本次残留，确认其被主图正常引用即无害。
- **② 复合目录全量对比**：`npx tsx tools/parse-gil-node-graph.ts <地图.gil> --list`——核对目录里
  **没有多版本残留**（同名 `(1)` 后缀副本、已删除复合的旧 def）。游戏校验目录里全部复合（含零引用残留），
  零引用错误副本照样拒载（2026-08-26 足球实证：删 9 个 (1) 版后游戏正常加载；2026-08-30 足球实证：
  7 个残留含互相引用链 auto_check_tick→dribble_decide，def-clean --force 整体删）。
...
- **只读工具会归一化差异**：scan/parse/explain 对"我的注入版 vs 编辑器修复版"可能输出一致但游戏拒载
  （2026-08-30 足球信号拒载实证：注册表 615B 一致、监听节点结构一致，差异在字节层）。判游戏能不能
  加载，最终依据 = 真实 .gil 与 .gia 的**原始字节窗口对照**（信号节点名 pin 形态见 verify-injection 8c）
