...
if (!GRAPH_ID || !SOURCE || !TARGET) {
  console.error('usage: verify-control-flow-experiment.ts <dir> --graph-id N --source N --target N [--outflow-index N] [--expected-pin-raw hex] [--before-hash h] [--after-hash h] [--source-generic N] [--source-concrete N] [--target-generic N]')
  process.exit(2)
}
