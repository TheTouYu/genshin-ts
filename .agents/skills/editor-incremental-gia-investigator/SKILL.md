...
description: Investigate and accumulate Genshin editor engine rules through user-saved, adjacent map snapshots. Always use this skill when adding a game-engine feature, studying how a level asset/node/pin/connection/parameter/component/UI/variable is encoded, comparing before/after GIL files, or preparing a learned rule for Genshin-TS implementation. Also use it whenever a bug investigation gets stuck or a production rule is not yet closed: the first move must be a minimal editor diff (10-second change) to learn the real wire, never static guessing in production code. It first reads the project's human-readable game-engine knowledge graph to reuse known concepts and evidence, then performs only the missing incremental experiment, preserves immutable snapshots and hashes, derives bounded rules, hand-replays them in temporary GIA/GIL, and keeps real-map writes behind explicit or scoped standing authorization.
...
- **旧编辑器内存保存覆盖是最常见"变更消失"假象（2026-08-16 灯阵复发实证）**：灯柱实体两次"消失"、信号 v4 出错版均由此产生——实体/图/信号结构其实已正确，用户旧内存保存覆盖了磁盘写回。排查"变更消失"前先核对当前 hash 是否等于写回后 hash；等于则先请用户重新加载再观察，不要直接怀疑结构、重做注入或开始差分。
...
| 屏幕 UI 控件（root9 控件段）：图片控件引用素材、控件记录结构、布局 packed 列表 | `references/ui-controls.md` |
...
## 发出变更请求前：必须先物理备份 BEFORE 快照（2026-08-14 教训）

**向用户发出"请去编辑器做变化"的请求之前，必须先把目标 .gil 物理复制一份到实验目录
（含 hash 记录），绝不能只记 SHA-256。** 用户保存后原文件即被覆盖，hash 无法还原内容；
编辑器自动备份（Save_Level/.gsts/backups）与注入备份（~/.genshin-ts/backups/<playerId>/<mapId>/，
gsts.ts maybeBackupGil，5 分钟去重窗口）都不保证覆盖"用户保存前的精确状态"。

流程：

1. 发出变更请求前：`cp <map>.gil <实验目录>/<map>.before-<hash8>.gil` + `sha256sum` 记录；
2. 用户保存后：复制 after 快照 + 记录 hash，再开始差分；
3. 若 before 已丢失（只记了 hash 没复制）：先查两处自动备份（.gsts/backups 编辑器备份 /
   ~/.genshin-ts/backups 注入备份），逐 hash 核对；都不匹配时不得猜测归因，立即停下
   报告，按用户指示处理（通常用户可再做一轮共享变更，当前状态作为 before）；
4. 注入/写回类任务的备份以"注入器自动备份 + 本规则的手动 before"双保险。
...
## UI 差分实战经验（2026-08-17/18，灯阵/UI 解析）

### 顺利点（可复用）

- **真实模板克隆法**：创建复杂 UI 控件不必从零编码——从真实样本记录克隆，只改索引 ID 和业务字段（内容/位置/尺寸），再登记进布局 packed 列表。`gsts assets:ui clone` 即此思路。
- **packed varint 列表识别**：`layout.503` 是一串 packed varint（控件 ID 列表），不要用消息解析器去解，直接用 `readVarint` 流式读；否则会出现“字段号 134217728”这类假消息。
- **深层位置字段**：UI 控件位置在 `12.501[n].502.f504`，是**屏幕中心相对偏移**；编辑器显示的是绝对坐标（1600×900 设计分辨率）。差分时用两个对角样本即可闭合坐标换算。
- **等长内容变更**：内容改成长度相同的文本时，控件记录总长不变、只有 root46 审计变化；要确认“是否真的改了”必须做**逐字节记录 diff**，不能只看 root 层大小。
- **跨图交叉验证**：两个地图轮着写/验，能快速暴露 CLI 编码与编辑器首存差异。

### 阻碍点（已沉淀为技能改进）

1. **旧编辑器内存覆盖假象复发**：用户“改了内容但没落盘”，实际是没重新加载地图就保存。规则已在 SKILL 里有，但 UI 场景再次触发——排查“变更消失”仍必须先核对 hash + 重新加载。
2. **通用递归解析会把 packed/原始字节误当消息**：做 UI 记录修改时，不能对所有 wire2 字段递归 parse；必须**只沿已知路径替换**，未知子块保留原始字节（否则字节漂移）。
3. **root21/root29 编辑器自动登记**：CLI 创建 UI 控件后，编辑器首存会自动补 root21（标记）和 root29（十进制字符串登记），root29 语义尚未闭合。CLI 无法一次写到位，需要“首存规范化”步骤。
4. **构建阻塞干扰**：`examples/rubik-2x2` 遗留语法/类型错误会卡住全量 `tsc`。差分/CLI 开发时应先确认主包构建是否被无关示例阻塞（可临时在 tsconfig 排除）。
5. **知识录入与主流程并发**：子代理做知识树录入时，`PLAN_WORKTREE_DRIFT` 等防护要求 apply 是提交单元；主流程的未提交改动会让知识 plan 受阻。最好先提交主流程改动再开知识 plan。

### 对差分技能的下一步优化建议

- ✅ 增加一个 `references/ui-controls.md` 模块（2026-08-22 已落地，含图片控件引用素材 wire）。
...
## 素材库注入差分实战（2026-08-22，UI 资产直接注入）

> 背景：AI 生成 UI 资产（CSS/SVG）→ 网页审美 → **直接注入游戏素材库**（用户 08-22 11:20
> 纠偏：不是导出 GIA 再手动导入，而是 CLI 直接写 .gil 素材库）。
> 证据全部来自真实 .gil 相邻快照差分（编辑器逐项操作 + 只读快照 + 定点比较），
> 关卡路径见 AGENTS.md。已闭合规则速查见 `references/library-injection-rules.md`——**本技能续作必读**。

### 顺利点（可复用）

- **整段克隆 + 编号重映射**：素材库记录是"容器/组/图元"骨架，注入 = 克隆真实样本段，
  重映射 ID 与互指字段（502.13/502.14），追加 num501 注册，root46 由编辑器首存补齐。
- **画布原点在中心**：元素位置 f504 是中心相对偏移；编辑器 (100,100) ↔ wire (0.0729, 0.0001) 逐字节验证。
- **两个素材库同构**：root9 append num501 ID + 注册 + 连续子资产段 + root46 噪声，第二次直接套用。

### 阻碍点（已沉淀为技能改进）

1. **闭合结论不落盘 = 知识蒸发**：11:43 差分全部闭合，12:06 才想起"需沉淀（收尾时做）"，
   期间 27 次上下文压缩——若会话中断，8 小时差分成果只剩会话日志兜底。
   → 规则：**每轮差分闭合的结论必须当轮写入 references/ 或 docs/game-engine-knowledge/，再进下一轮**。
2. **记录顺序假设未早期验证**：默认"追加末尾保持升序"，12:38 才发现真实 .gil 素材段
   **记录顺序非升序**（新块在前、旧记录在后，疑似早期实验残留）→ 插入位置规则返工。
   → 规则：**差分早期必查"记录顺序/插入位置"（dump 全段 ID 序列），不要假设追加语义**。
3. **wire 细节不能凭常规推断**：504/505 内部字段号是 501/502（不是 1/2）；502{13} 是 w0 裸
   varint、502{14} 是 w2 包裹裸 varint（不对称）；浮点子字段是 wire5 fixed32。
   → 规则：先逐字节读真实样本再写生成器；测试断言用真实样本的精确字节。
4. **API 层 400 与任务无关但最打断**：reasoning_content 回传缺失 400 三波 29 次，
   09:03→09:53 中断 50 分钟（用户手动"继续"才恢复）→ 应对见 `docs/collaboration-handbook.md` A11。

### 未闭合（续作入口）

- 真实 .gil 素材段记录顺序的**来源与结构**（为何新块在前）——dump 各块归属后再定插入规则。
- `library_inject.ts` 候选注入验证（`--output` 候选 → 独立回读 → 用户确认 → `--write`）。
- 素材库规则正式入 PKC 知识树（knowledge/game-engine-knowledge/，走 tools/pkc.py bundle 流程）。
