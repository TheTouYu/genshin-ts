...
## 游戏拒载检查清单（2026-08-16 信号五连错实证——按顺序执行，每步有明确命令）

游戏"参数错误/启动失败"、级别极高进不去、Beyond_Debug_Log 无新文件时，按此顺序排查。
**不要跳步**——5 次错误的根因分布在每一层，静态检查全过不等于引擎认可。

```text
① 确认现象层级：进不去游戏 = 加载期错误（无日志，正常）
   → 不要等日志，直接进入差分流程

② 版本一致性：注册表条目 f6 == 三份 CompositeDef #4 最后 field5
   命令：npx tsx /tmp/verify-v3-map.ts <map.gil>   # 输出 v= / defs=[..] 必须相等且 ≥3

③ 版本下限/阈值：v ≥ max(3, ceil(3N/4))，N = 被引用信号数
   → 版本 2 必拒（vec3）；v3 在 N=5 时拒；register 已全表回填只升不降

④ 参数 n3 field2 序号（第 5 次根因，最隐蔽）：
   send/server 参数 n3 field2 = 全局序号（0 省略）；monitor = 3 + 序号
   → 怀疑时：用修复后 CLI 重新 update 该信号，候选 hash == 当前地图 hash 即布局正确；
     或用编辑器重建样本逐字节对照

⑤ 信号节点参数默认值（第 4 次根因）：
   发送信号已连接 InParam 不得带默认值（vendor 对 null Vector 自动补空）
   → 检查图内信号节点：已连接 pin 的 value 必须为 null
     （普通 SysCall 节点带默认值引擎容忍，不要误改）

⑥ 客户端 send_signal_to_server 参数默认载荷（未闭合风险）：
   形态与编辑器样本一致但从未端到端验证；游戏拒载时列入怀疑

⑦ 排除"变更消失"假象：
   CLI 写回后用户旧编辑器内存保存会覆盖磁盘 → 先核对当前 hash 是否等于写回后 hash；
   等于则请用户重新加载编辑器再观察，不要重做注入
```

每步结论记录：检查了什么、命令输出、通过/失败。任何一步失败即为根因方向，
修复后用**生产工具独立跑完整流程**（register→注入→游戏）验证，不能靠用户手工中间产物。
