...
- **客户端「获取自定义变量」帧判据（2026-08-30 3007/3008 受控差分实证）**：变量存在（预注册）=
  `head=04: IN0:Entity=玩家 | IN1:String=变量名 | OUT0:Integer=值`，值流入下游信号参数
  （`IN1:Integer=值`）；**变量不存在（服务端动态创建）= 同节点只有 IN0/IN1、无 OUT 行**，
  下游信号参数 `IN1:None=?`，服务端 monitor 不触发。客户端记录 f21 大小突变（同图
  134B↔156B）+ head=04 有无 OUT 行 = 变量跨端可见性一眼判据（详见 debug-log-format.md）。
...
- 变量初始值**不在**日志里（只记录执行帧）
- **负整数双显示坑（2026-08-27 2931 实锤）**：同一负值在日志不同字段显示不同——`Integer` 字段显示 uint64
  无符号（如 -2 = `18446744073709551614`），`List` 元素字段显示 uint8（如 -2 = `254`）。**解析脚本必须用
  BigInt 处理**：`if (v > 9223372036854775807n) signed = Number(v - 18446744073709551616n)`；`Number()`
  直接转 uint64 会丢精度（`18446744073709551614 → 0`）导致误判为 0/moveId 丢失。切勿把 List 里的
  254/255/253 当独立值——它们是 -2/-1/-3 的 uint8 显示。
