...
- **新资源类型/新厚度先做颜色小样**（2026-08-09 魔方 3x3 第二轮教训）：wire 编码与 export 回读一致不代表游戏/编辑器一定显示。`10009001` 长方体装饰物闭包颜色在用户侧显示全黑（与棱柱候选编码完全相同，根因未闭合），正式模型前先放单资源颜色小样校准板（不同颜色 × 厚度 0.02/0.05）请用户确认，不要把未核验颜色的资源直接堆进完整模型。
...
- **10009001 长方体 scale=1 可能是 2×2×2（半尺寸 1.0），不是 1×1×1**（2026-08-09 魔方第三轮用户游戏实测：隐藏体块后 0.52 偏移的彩面落在体块“中心”附近 → 面片实际在表面 1.0 内部 0.48，与 2×2×2 自洽；颜色渲染已由校准板证明正常，全黑不是颜色问题）。旧“1×1×1”与棱柱“边长 1”同源目测误判（棱柱已修正为外接圆直径 1）。**做任何魔方/方块类模型前必须先小样校准长方体真实尺寸**（放 scale=1 长方体 + 已知参照对比），不得沿用 1×1×1。面片外贴位置应为“真实表面 + 半厚 + 间隙”，不可直接抄 0.52。
