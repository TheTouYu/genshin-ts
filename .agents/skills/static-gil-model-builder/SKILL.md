...
- **向已有地图增量添加新资产**：`assets:static-assemblies` 不能更新已存在 prefabId 的闭包，用**新 prefabId + 新 aux 区间**增量添加（候选基于当前地图只加新闭包，既有闭包完整保留）；用户打开编辑器保存后源 SHA 会变，写回前重取。**而且不止 SHA：用户可能用编辑器重导入/重排实体，ID 占用分布整体变化**（V5 实测：V4 场景实体被用户重导为另一个 ID、其克隆 aux 区间整体后移）——每轮开始时重做 inspect + entities export 双查，任务文件/历史记录里的 ID 表只当线索，以实时盘点为准。**唯一铁证 = 直接扫真实 .gil 的 root27 双侧记录 ID**（2026-08-23 灯阵 L4/L5/L6 实证：计划文件里 aux 最大 1073741948，但真实地图 root27.2 还有 1073741949-1952 的编辑器手动装饰物 → plan 报 `assembly IDs conflict`；`resources list`/`entities export` 都列不出它们。扫描法：payload = `source.slice(20, -4)` → `parseWireMessage` → `wireRecords(top,27,1)` 与 `(27,2)` 各取 `wireRecordId`，取两侧并集最大值 +1 作为新 aux 起点）。
