...
## 足球视觉规范（用户已验收的视觉方向，2026-08-09 评测轮修正）

**结构**：空模型宿主 + 12 个五棱柱（`10009005`）+ 120 个三棱柱（`10009004`，每六边形 6 个三角形）。

**用户验收过的关键视觉规则（2026-08-09 修正，含用户游戏内实测语义）**：

1. **资源语义（重要）**：三棱柱/五棱柱等基础元件 scale=1 时底面**外接圆直径 1**（外接半径 0.5）。目标外接半径 r 的 `scale.x/z = 2r`；目标边长 s 的三角形 scale = s/0.866、五边形 scale = s/0.588。
2. **六边形内部必须无缝**：6 个三角形拼一个完整正六边形，三角形之间绝对不允许有间隙。
3. **五边形与六边形边长相等（共边）**：R=1.0 截角二十面体边长 0.4035，则五边形 `scale=0.6866`、三角形 `scale=0.466`，无缝共边。旧 v2 参数（五边形 0.3105/三角形 0.2381 @R=0.5）按新语义分别是共边值的 -9.5%/+2.2%——即用户反馈的“五边形略小”。若视觉需要遮缝，在共边值上小幅放大（如 +2~5%），不做大补偿。
4. **面片间允许微缝当黑线**：五边形与六边形之间的细小间隙视觉上像黑色缝线，可接受；间隙过大则必须用黑色线条/填缝处理。
5. **颜色禁止纯黑纯白**：已验证刺眼。用户满意的 v2 色值：五边形 `rgb=1514018`（0x171A22 深灰蓝黑）、三角形 `rgb=12107976`（0xB8C0C8 米灰白），`enabled:true/opacity:100/overwrite`。（十进制为用户游戏内实测权威值，括号 hex 为换算标注；1514018=0x171A22、12107976=0xB8C0C8，勿再抄写旧标注 0x171C22/0xB8B8A8。）

**已闭合（2026-08-09）**：10009005/10009004 的 scale 语义 = 外接圆直径 1，用户游戏内实测比对确认；v2 的 0.3105/0.2381 与新语义完全自洽（-9.5%/+2.2%），解释了“满意但略小”与“小了两倍”两个历史反馈。
