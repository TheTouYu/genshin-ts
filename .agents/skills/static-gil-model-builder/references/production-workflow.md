...
- **maps / entities export 顶层键名速查（2026-08-12 复盘：cube-blocks5 曾逐次探测）**：`maps --format json` 顶层 `{schemaVersion, kind, maps}`，每项 `{mapId, name, modifiedAt, modifiedAtMs, size, recent, locator}`；`assets:entities export --format json` 顶层 `{entities, schemaVersion}`，每项含 `{id, name, definitionId, resourceId, position, rotation, scale, color, components, auxIds}`。
