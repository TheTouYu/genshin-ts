...
- **`start_timer` 延迟列表不要用 0.0**：0s 定时器实测不触发（2026-08-20 3×3 魔方：execMove 用 0s 汇聚定时器，日志只有 Start Timer 帧、没有 When Timer Is Triggered，导致锁卡 true 无反应）；需要“立即/下一帧执行”时用 `[0.01]` 等小正数。
 - **物理链设计：状态迁移必须显式启动物理链，事件分发按状态不按设备名（2026-08-30 足球断链实证）**：若物理推进依赖“运动器停止事件”，任何状态迁移后若球上无对应设备在跑即断链（脱脚后球冻结：最后的 dribbleCtrl 停止被旧分发忽略）。规则：
   - 状态迁移处理器里**显式启动**新状态的物理链（如 CARRIED→ROLLING 直接执行第一次 roll tick 或激活 physics 设备 0.2s），不等待“碰巧的设备停止”；
   - whenBasicMotionDeviceStops 分发按 **state**（物理态任何设备停止都恢复物理链，控制态 CARRIED/GOAL 忽略），不按设备名过滤——唯一名冲量设备（“1”/“2”…）的停止也必须触发物理 tick；
   - 修复标准对照**真实物理语义**（状态迁移 ⇒ 物理规则立即生效），不要延续旧代码“能跑”的怪癖（旧分发器只响应 name==physics 是断链根因）。
