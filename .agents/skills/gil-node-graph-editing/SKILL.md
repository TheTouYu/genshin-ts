...
6. **客户端图先映射真名再动手（2026-08-28 魔方客户端图复盘同族扩展）**：目标图 type≠20000 时
   （20010 角色操控技能图等），read/parse 输出的节点名是服务端撞号错标名——先按
   gil-node-graph-reading 技能 Step 2.8 用 CLIENT_NODE_METADATA 映射真名，否则按错名加节点/
   连引脚必写坏图。
