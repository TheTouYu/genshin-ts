...
## 检索小技巧（多用关键词提升召回，2026-08-19 实测）

**核心：一次查询并多个不同层面的关键词，召回显著高于单关键词。**

- **并 3~5 个不同层面的词**：领域词 + 专有名词 + 中英混排。例：`rag_search(["仇恨 嘲讽 追击 怪物"])` 好过 `["仇恨"]`；`get_node_info(["嘲讽目标","增加指定实体的仇恨值"])` 一次批量。
- **用已知专有名词当关键词**：节点名、参数名、官方术语（如 `嘲讽目标`、`仇恨值`、`激活`）命中率最高；模糊说法换成官方术语再查。
- **英文技术词也带上**：部分文档用英文术语，中文查不到时并上英文（如 `GetVar / SetVar / LocalVariable`）。
- **查不到先换词再判缺**：按顺序试 ①更短/更通用词 ②同义词 ③英文/官方术语 ④领域术语重写整句；仍无才走本地回退，不要反复重试同一远程 API。
- **本地 PKC 知识树也可查**：项目本地 PKC 树（307 claim，含官方指南/UGC 一图流）用 `python tools/pkc.py knowledge-search "词1 词2 词3"`，同样用多关键词技巧；本地树覆盖远程 KB 可能缺失的规则细节。
