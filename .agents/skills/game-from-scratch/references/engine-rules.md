...
- **负载与时序稳定性（2026-08-14 用户定义+核验）**：引擎有单位时间运算量上限（负载），长时间高频操作可能瞬时触顶 → tick 延迟/卡顿。**"完成后才能继续"的时序必须用相对触发**（运动器实际触发后嵌套延时解锁），绝对时刻（固定 1000ms 解锁）在负载高峰后立即放行会中断未完成的运动器 → 永久偏差。相对时序即使卡 1 秒，恢复后仍按实际时序完成，行为稳定。- 定时器终止不可恢复：`terminateTimer`；暂停可恢复。
