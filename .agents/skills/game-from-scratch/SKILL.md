...
### 物理状态机玩法（2026-08-23 足球实证，刚体球完整物理）

刚体物理（球）用**状态机**表达：`state` 图变量 0=静止 / 1=空中 / 2=滚滑，外层 `physTick` 只做状态分派，每个状态一个独立 tick 复合。关键教训：

1. **贴地状态不积分重力（最容易踩的坑）**：滚滑状态若也套含重力的三力积分，球会持续"重力下沉 → 地面反弹拉回 → 再下沉"循环**微弹**（视觉像在草地上颠簸，不像贴地滚）。正确做法：贴地滚滑用**独立积分复合**——只做水平摩擦（v×μ/tick）、`vy=0`、`y` 锁定贴地高度；重力被地面支持力抵消，**不积分重力**。
2. **空中三力积分**：`a = 重力 + 空气阻力(-Kd·|v|·v) + 马格努斯(Km·(ω×v))`，旋转衰减 `ω·=exp(-k·dt)`；`p += v·dt`。
3. **碰撞精确反射**：标准公式 `v' = v - (1+e)(v·n)n`（e=恢复系数，n=碰撞面法向）。**不要用"分量各自反向"**（擦边球会把切线方向误反向）。法向按碰撞体几何：垂直圆柱（门柱）用水平法向 `normalize(dx,0,dz)`；水平圆柱（横梁）用 `normalize(dx,dy,0)`。DSL 实现 = `normalize + dotProduct + zoom + addition` 四个向量节点，避免手写分量反射。
