...
| Client node support | `docs/architecture/client-node-support-plan.md`; encoding details: `docs/architecture/client-gia-encoding.md` | `src/runtime/IR.d.ts`, `src/runtime/client.ts`, `src/compiler/client_layout.ts`, `src/compiler/client_ir_to_gia.ts`, `src/compiler/signal_registry.ts`, fixed vendor client snapshot | two real 6.7 client skill GIA WP0 baselines, scalar/entity-GUID/full-list signal reference, `tests/runtime/test-client-*.ts`, `tests/composite/test-client-signal-materializer.ts`, editor import/export structural diff, game behavior verification | Current production scope is skill signal/list/data-source + shared layout; signal identity comes from target GIL; no client injection or target-ID guessing; new node families require separate evidence |
