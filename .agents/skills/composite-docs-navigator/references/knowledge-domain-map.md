...
| Client node support | `docs/architecture/client-node-support-plan.md`; encoding details: `docs/architecture/client-gia-encoding.md` | planned: `src/runtime/IR.d.ts`, `core.ts`/client registry, Stage 1 client context, Stage 3 client lowering, fixed vendor client snapshot; current focused signal materializer: `tests/composite/test-client-signal-materializer.ts` | two real 6.7 client skill GIA WP0 baselines, scalar/entity-GUID/full-list signal reference, focused GIA tests, editor import/export structural diff, game behavior verification | Production `g.client()`/Client IR/Stage 3 support remains gated; signal identity comes from target GIL; no client injection or target-ID guessing |
