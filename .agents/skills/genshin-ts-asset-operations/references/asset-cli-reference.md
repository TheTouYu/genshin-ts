...
**⚠️ 预注册前置红线（2026-08-30 用户定规 + 3007/3008 受控差分实证）**：DSL 里 `setCustomVariable` /
`getCustomVariable`（尤其客户端图读取）使用的自定义变量，**必须先在本节 CLI/API 预注册变量定义，
再进 DSL**。依据：①官方文档「设置自定义变量…要求目标实体上存在自定义变量组件，且有对应名字
的变量，否则节点无法正常执行」；②3007→3008 受控差分定论：未预注册、靠服务端运行时"动态创建"的
变量（`d2c_counter`）——服务端当 tick Set→Get 读回正常，但客户端图「获取自定义变量」**无 OUT
输出帧**（信号 val=None、monitor 不触发）；预注册后**编译产物零改动**即全链闭环
