...
### 7.1 外部资源包：社区「常用复合节点大全」（2026-08-22 学习）

社区作者「左岸丶寒」整理的 87 个通用复合节点（13 类资源包），已逆向整理到
`docs/composite-library/`（README 是总览，各资源包文档含用途/节点清单/通用方法论/复用提示）。
这是**「从外部资源包提炼通用知识」的第一次实践**，与第 7 节「从自己项目提炼」互补：

- **第 7 节**：从 rubik 项目提炼（A/B/C 三档，按「可复用性」分）；
- **第 7.1 节**：从社区资源包提炼（13 类，按「功能」分）。

两者共同构成「复合节点资产库」的两个来源。提炼出的**跨资源包通用方法论**（三段式骨架、
动态列表转静态、哨兵值、空模型当结构体、枚举转值、负载意识、复用前置条件）已回灌到
`dsl-nodegraph-development` 技能「通用复合节点模式库」一节。
