...
### 7.1 复合节点资源库：社区资源包 + rubik 项目抽象（2026-08-22）

`docs/composite-library/` 现共 **19 类资源包**（README 是总览，各资源包文档含用途/节点清单/通用方法论/复用提示）：

- **1-13 类**：从社区「常用复合节点大全 v1.7」（作者「左岸丶寒」，87 个通用复合节点）逆向整理，按功能分。
- **14-19 类**：从 rubik-2x2/3x3 项目抽象（数学/几何、长列表、输入锁、跨图同步、定时器调度、循环物化）。

这是「复合节点资产库」的两个来源（社区资源包 + 自己项目提炼），两者共同构成可复用资产。
提炼出的**跨资源包通用方法论**（三段式骨架、动态列表转静态、哨兵值、空模型当结构体、枚举转值、
负载意识、复用前置条件）已回灌到 `dsl-nodegraph-development` 技能「通用复合节点模式库」一节。

**关键认知**：资源包里复合节点的**名字在 `comments` 字段**（作者写的中文注释），`name` 字段是空的。
**注释是资源包的核心价值**——没有注释的复合节点无法复用。写复合节点时，注释要写清：
用途 / 依赖（挂载哪、需要哪些变量）/ 负载 / 注意事项。
