...
**玩家/角色实体（v9 差分 + 2026-08-29 游戏核验矩阵批次 11）**：玩家实体 f8=def=1000000
（默认模版，多人每玩家一个，本样本 1086324743/44/45 各自持有一份同名变量「这是玩家变量」）；
角色实体 f8=def=1000001（1090519041「默认模版(角色编辑)」）。两者的变量容器同样是 `f7.{f11}`、
entry 编码与关卡/普通实体逐字节同构；角色实体组件槽更多（8 个 f7）。
**两层语义必须分开（用户 2026-08-29 纠正）**：
- **资产侧（代码实现，矩阵批次 11/12 字节实证）**：编辑器里看到的多个玩家实体（本样本
  1086324737–745 共 9 个副本，非 3 个——2026-08-29 批次 12 修正）只是资产/模板实现——修改任一
  玩家实体变量，编辑器自动同步到全部玩家实体副本（entry 字节完全相同 = 模板复制）。这是编辑器
  的代码实现行为，**不代表**游戏运行时有多个玩家。
- **游戏行为（用户知识，L4 待游戏内核验）**：游戏未开始没有"多个玩家"概念；引擎运行时修改
  玩家变量，游戏开始后**不管实际游玩人数（最多 8 最少 1）都对所有玩家生效**；玩家实体上挂载
  的节点图每个玩家实例各触发一次。**关卡实体（def 10003004）游戏开始创建一次，不重复触发**
  ——与玩家/角色的 per-player 运行时语义不同。
**空字符串默认值规则**：
...
## 客户端局部变量（按名字，2026-08-29 v15/v16/v16b 差分闭合）

> 状态：已验证（编辑器样本 var-v15-client-graph.gil sha e12fc8d7… / var-v16-client-more-types.gil
