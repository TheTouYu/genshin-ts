...
> ⚠️ **结论修订（2026-08-16 五连错第 5 次差分实证，commit 9e8fd76）**：
> 本节"`BUILTIN_PARAM_LAYOUTS` 类型值布局 = 引擎可加载的规范布局"的**结论已作废**。
> 08-15 的"规范"判断只基于编辑器样本字节对照（低证据，且该节自己注明游戏核验未完成）；
> 08-16 三信号差分 + 用户游戏核验证明：**引擎接受的 field2 = 参数全局序号**（见
> 「信号参数 n3 field2 = 参数全局序号」节），builtin 类型值常量（vec3=2/int=0/…）全部错位。
> 本节保留的仍有效事实：①1849/1888 新建样本参数段 field2=类型值（字节观察，非规范依据）；
> ②`repairSignalInGil` 需同步重建注册表索引条目（pinIndex 修复，与 field2 无关的另一层）。
> ③"编辑器内手动追加参数"仍是破坏性操作（应重建信号而非追加）——但 08-15 把"追加路径不可加载"
> 归因于"序号式布局"本身，该归因不成立：08-16 证明序号式正是引擎接受的形态；追加路径当时的
> 不可加载更可能来自追加操作的其他破坏点（pinIndex 重排等），差异未完全闭合。
> 08-15 单参数游戏核验（日志 2707）与本节结论冲突之处无法回溯（当时未逐字节记录注入定义的
> field2 实际值），以 08-16 规则为准。
...
结论：**版本 >=3 且两边一致即可加载**；当时另写"布局（类型值/序号式）与版本精确值（3/4）
不敏感"是**过度归因**——用户修复同时改了版本（2→3）与布局（→序号式）两个变量，该样本不能
单独归因布局；08-16 三信号差分证明布局（field2 序号）敏感（见「信号参数 n3 field2」节）。
builtin 模板此前复刻 verify_ping（str,str，版本 2）——str 版本 2 历史可加载（U1 实验：
2026-08-16 监听触发差分实验，日志 2699，见上文「尚未闭合」节修订记录），
...
## 信号版本动态阈值 f(N)=max(3,ceil(3N/4))（2026-08-15 灯阵第 3 次错误实证，commit 66e24d9）

**规则**：信号版本不仅要 >=3 且两边一致，还要满足**随被引用信号数增长的动态阈值**：

```text
f(N) = max(3, ceil(3N/4))   # N = 被引用信号数
      （等价：注册表 identity 记录数 3N / 4 向上取整——每个信号注册生成 3 条 identity 记录）
```

**第 3 次错误**：N=5 信号时 v3 < 阈值 4——逐个 register 旧信号版本停留偏低（v3），
新信号加入后未全表回填 → 引擎拒载。修复：`register`/`update` 版本取
`max(target.v, versionFloor(N))`（只升不降），register 对全表回填。

**源码**：`src/cli/gil_signal_registrations.ts` `versionFloor`（≈L739-744）。
**未闭合**：f(N) 是否含"上限"未验证（v5/v6 在 N=5 可启动；更大 N 未测），
见复盘文档风险表。
...
**术语桥接（与第 4 次复盘对照，避免字面矛盾）**：第 4 次复盘（`retrospective-2026-08-16-signal-param-default.md`）
中的 `i2.index`（如 `{3,0}`/`{3,2}`）是**图内信号节点**的参数 pin 描述符（节点侧序号）；
本文的 n3 field2 是**注册定义**（CompositeDef #102/#103 参数条目）的描述符（定义侧序号）。
两者是不同层字段：第 4 次"i2.index 不是问题"指节点侧描述符正确，第 5 次"n3 field2 错位"
指定义侧条目错位——不是同一字段的相反结论。排查时两层都要核对。
