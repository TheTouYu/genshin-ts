...
`CONFIRMED`：技能 ID=1228931073；创建（2a）与绑定（2b）的 root 6/15/16 均经 `/tmp` 逐字节
同构重放与 after 一致。客户端图引用字段 = `root15.4[1].30.1.5 = {f1:图ID, f2:1073741825}`
（f2 恒为 1073741825，与参考地图 1082130436 样本一致，语义 INSUFFICIENT）。技能类型普通/瞬发
= `4[1].30.1.1.4`（普通=2，瞬发=字段缺失）。事件轨道打点结构语义 INSUFFICIENT（f45.f2 计数
61→62 与默认条目移除绑定一起发生，未单独隔离）。
