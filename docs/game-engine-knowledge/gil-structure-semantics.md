...
客户端图 20010（CharacterControlSkill）folder typeValue = **7400**（2026-08-29，
1073741914 真实相邻快照 + `/tmp` 同构重放后 root 6/10 与 after 逐字节一致）：
新建 20010 图 = root 10 在既有图 field 1 记录之后追加 `{1: NodeGraph}`（NodeGraph 含
1 个自动「节点图开始」节点 `genericId.nodeId=200042` + `entrySlotIndex=1`），root 6
「未分类页签」f3 末尾追加 `f5={1:7400, 2:图ID}`。细节见 `node-graphs.md`。
...
| root | 静态结构（v21） | 状态 |
| --- | --- | --- |
| `6` | 页面/分类树：`6.1[*] = {f1: 组ID, f2: {f1:'root', f3:1}, f3: {f1: 组名UTF-8, f3:2, f5[*]: {f1: typeValue, f2: id}}}`；组名样本：`root/默认/未分类页签/默认分类/玩家模版/职业`；typeValue 100/400/200/800/7400 条目规则与已闭合结论一致（7400=client 图 20010，2026-08-29 快照闭合） | 结构 `CONFIRMED`（与实体组登记规则互证） |
