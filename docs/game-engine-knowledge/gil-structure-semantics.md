...
自定义造物技能模板（f2=28，2026-08-29 v10→v11）：瞬发释放，body 紧凑（`4[1]` 仅 26B，
`{1:73, 78:{1:{1:{1:20001},2:{1:10007001},3:{4:1,5:1}}}}`），事件轨道已简化（f45.f2=52）。
**创建时联动生成 root 20（1970B）= 造物模型容器**：骨骼节点（GI_MonsterRoot/GI_RootNode/
GI_Chest/Hand/Foot/NamePlate/HeadMark/AimingPoint 等）+ 受击/被击倒特效引用。root 20 是
本任务新发现的根容器，造物模型装配属编辑器联动（CLI 是否复刻列为 open-item）。
