...
#### 实体挂装饰物（2026-08-06，v11/v12 快照，v12 已注入待游戏核验）

实体（root 5）可以挂装饰物（贴片），与元件的装饰树同构，但只有实例侧：

- root 27 新增**实例侧**辅助记录（f2 子记录）：`f1=ID、f2=资源 ID、
  f4` 名称槽（f1=1/f11 名称 + f1=40/f50 挂接槽 = `[4016, ownerID]` varint 流 +
  f1=111/f93 空）、`f5` transform 槽（f1=1/f11 position/rotation/scale，
  稀疏编码）+ 材质槽 `f1=22/f32`（同实体颜色编码）+ f1=5/f15={1,1}、
  f1=2/f12、`f12=空`（实体挂载无定义侧配对，区别于元件拼装的 def-aux 配对）。
- 实体 field 5 的 `f1=40` 槽 f50 = `[4010, len, auxID×N]` varint 流
  （多个贴片拼一个列表；len = auxID 列表字节数）。星球样本（1 装饰物）：
  `aa 1f 05 81 80 80 80 04`；cube3 实例样本（3 贴片）：`aa 1f 0f 8a...`。
- root 22 属性注册表新增 `PropertyAttachArchetypeModel` 条目（f2 字节数随条目数增长）；
  root 46 是 5 槽 LRU 登记表（编辑器自维护，写回不碰）。
- 4010/4016 前缀语义 INSUFFICIENT（推测挂接类型码）。
...
- **删除装饰物**（v13 星球样本）= aux 记录保留但资源换成空模型 10005018，
  并新增 `f4{f1=20}` 槽（内容 f29 空）；实体 f1=40 挂接槽保留原样。
- 批量生成脚本：`.local/tmp/generate-cube-stickers.ts`（v12 魔方 54 贴片，
  已按 v13 规范更新，输出与 v13 逐字节同构）。

证据快照：`create-entity-v11/raw/`（星球实体挂 1 装饰物）、
`create-entity-v12/raw/`（魔方 54 贴片注入，待游戏核验）。
