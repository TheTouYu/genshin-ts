...
技能体 4[1].30.1 的子结构：
        ...
        4 = {2:1}
        5 = {1:客户端图ID, 2:1073741825}          # ★ 节点图引用（绑定客户端图后出现）
...
`CONFIRMED`（仅单图绑定样本）：技能 ID=1228931073；创建（2a）与单图绑定（2b）的 root
6/15/16 均经 `/tmp` 逐字节同构重放与 after 一致。客户端图引用字段 = `root15.4[1].30.1.5 =
{f1:图ID, f2:1073741825}`（f2 恒为 1073741825，与参考地图 1082130436 样本一致，语义
INSUFFICIENT）。技能类型普通/瞬发 = `4[1].30.1.1.4`（普通=2，瞬发=字段缺失，其余取值未采样）。

`INSUFFICIENT`（续作缺口，用户 2026-08-29 明确）：**一个技能支持绑定多个节点图**——多图绑定
编码未闭合（f5 是否重复 / 事件轨道 f45 如何多打点，均无样本）；事件轨道打点结构语义未闭合
（f45.f2 计数 61→62 与默认条目移除绑定一起发生，未单独隔离）。
