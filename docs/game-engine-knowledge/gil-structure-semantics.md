...
**静态元件（动态 → 静态转换，2026-08-20 真实样本 `after-convert-10009008-static.gil`）**：
- 用户把官方动态元件“圆柱 10009008”转为静态资源后，root 8 instance 记录从 543B → **409B**
- 差异：**去掉 f7 组件槽**；f6 槽1（transform）比动态多 10B 的“静态标记/元数据”
- 其余 f5×10 能力槽、f6×15 节点树、f8=resID 保留；root 6 仍登记 `{400, instanceID}`
- 静态资源只支持基础 缩放/位置/旋转；**不支持组件、变量、高级功能**
- CLI：`assets:prefabs create --static --base <官方ID> --id <new>` 生成同构静态实例
