...
造物技能给造物用，所以带造物模型选择。
**切换释放类型会自动丢弃节点图挂载 + 重置造物模型**（引擎正常行为，v12→v13 实测：绑定 f4
消失、事件轨道恢复默认、root 20 从 2955B 回退 1970B、模型引用 10007001→10005001）。
**创建时联动生成 root 20（1970B）= 造物模型容器**：骨骼节点（GI_MonsterRoot/GI_RootNode/
GI_Chest/Hand/Foot/NamePlate/HeadMark/AimingPoint 等）+ 受击/被击倒特效引用；绑定节点图时
root 20 再增长（1970→2955B，模型↔图联动）。root 20 造物模型装配属编辑器联动（CLI 是否
复刻列为 open-item）。
