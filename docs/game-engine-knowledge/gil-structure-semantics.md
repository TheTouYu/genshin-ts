...
技能配置资产（skill config，2026-08-29，1073741914 真实快照 + `/tmp` 同构重放，2a 默认创建）：

```text
root 6  重写「未分类页签」record（本图 folderId=68），f3 末尾追加 f5={1:7500, 2:技能配置ID}
        # 7500 = 技能配置 folder typeValue
root 15 追加一条 field 1 记录 = 技能配置资产：
        {1: 技能配置ID, 2: 36(普通技能), 4[*]: 分节}
        4[0] = {1:1, 11:{1: 名称UTF-8}}            # 名称（本样本「操控技能」）
        4[1] = {1:21, 30:{ 技能体 }}                # 技能体（大量默认参数 + packed 502 列表）
        4[2] = {1:35, 45:{ 节点图事件轨道 }}         # 事件轨道（默认含「节点图事件轨道」条目）
        4[3] = {1:36, 46: 空}
root 16 追加一条 field 1 记录 = 平行引用（同 ID 双写）：
        {1: 技能配置ID, 2: 技能配置ID, 3[*]: 与 root15.4[*] 同构，但 3[0].11 名称为空}
```

`CONFIRMED`（2a 默认创建）：root 6/15/16 与 after 逐字节一致；技能 ID=1228931073、
type=36（普通技能，用户确认）。`INSUFFICIENT`（2b 待闭合）：客户端图引用字段——默认未绑定图，
参考地图 bound 样本显示客户端图 ID 出现在技能体 `4[1]` 内（`f5={f1:图ID, f2:轨道ID}` 形态），
待用户绑定差分确认。
