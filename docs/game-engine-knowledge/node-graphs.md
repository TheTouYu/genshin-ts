...
### 定时器事件（When Timer Is Triggered，gid 83）输出顺序（2026-08-30 双证闭合）

> 状态：已验证（真实 GIA 帧 + 用户编辑器差分双证）
> 样本：`Beyond_Debug_Log/2026-08-30_06-57-51_2998_110170759.gia`（图 1073741828/1829）、
> `Beyond_Local_Save_Level/1073741915.gil`（差分前后快照 `var-v17-pre-diff-add-IN1-2026-08-30.gil`）

**官方/编辑器定义顺序**（vendor `server_event_en-us`、编辑器内置定义一致，用户编辑器实测）：
`事件源实体, 事件源GUID, 定时器名称, 定时器序列序号, 循环次数`（序号 index 3）。

**引擎运行时输出顺序相反**（帧参数自报 index 铁证，2998 日志）：`OUT3 恒空、OUT4 按定时器各自计数`（d2lv:1,2,3 / d2skill:1,2,3，每定时器独立序列）——**运行时序列序号在 index 4**。

**结论与用法**：
- 连线按引擎运行时顺序解释：`timerSequenceId` 必须连 **index 4**；连 index 3 运行时读空（d2lv|dyn| 恒 0 实测）。
- 编辑器打开我们的图时，该连线显示在"循环次数"输出上（显示错位、运行正确）——引擎定义与运行时不一致的已知怪癖，勿按编辑器显示改回。
- 编译产物顺序由 `scripts/generate-definitions.ts` buildEvents 的 whenTimerIsTriggered override 保证（vendor 保持官方顺序）。
- 该节点**无定时器名称过滤**：同一实体上任意定时器触发都会进入执行（1828 图在 d2skill_tick 上也执行，OUT2=触发定时器名）——需要按 `evt.timerName` 自行分流。
...
- 客户端图 ID 段起始值 = **1082130433（0x40800001）**：用户 2026-08-29 从初始地图实测
  （第一个客户端节点图 ID 即 1082130433）；1073741914 连续建 20010→20002→… 取
  1082130433 → 1082130434 → …（+1 递增，至 1082130440）；参考图 20010 为 1082130436。
  自动分配 = 客户端段内 max+1（段空=1082130433）；`assets:node-graphs create --type` 已按此
  自动分配（`--graph-id` 可显式覆盖）。跨类型/跨保存的全局复用规则仍 INSUFFICIENT（不复用空洞）。
