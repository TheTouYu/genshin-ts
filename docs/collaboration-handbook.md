...
- **卡住时的三问（2026-08-14 #17 教训）**：①知识库/文档是否已有此规则（有→按规则实现）；
  ②能否让用户做一个 10 秒编辑器最小差分学真实 wire（能→先差分，不要在代码里考古）；
  ③现有状态是否可做天然实验（注入版对比、编辑器保存版直接测试）。
  本次 v16 启动失败曾在 composite_registry 静态推断数小时——用户 10 秒差分直接给出规则。
- **分层归因（用户要求）**：出问题先分三层——①是否按生产代码语义编写（用法层）
  ②设计的 DSL 语义本身是否正确（设计层）③生产代码是否符合学到的复合规则（实现层）。
