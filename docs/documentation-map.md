...
> **PKC 检索语义（2026-08-16 实测补注）**：`pkc progressive-query --context <id>` 只接受
> **已注册的检索 context**（当前：`compiler-diagnostics`、`static-gil-assembly-production`，
> 对应 `project-intelligence/contexts/*.md`），知识树节点 id（如 `composite-boundary`）不是检索
> context，会报 `RETRIEVAL_CONTEXT_UNKNOWN`；context 正确但该 intent 无命中是正常结果
> （`RETRIEVAL_CANDIDATE_UNKNOWN`/coverage_gap），按 Adapter 转最小 Authority fallback 或
> `pkc knowledge-search`（lexical，总是可用）/ `pkc tree` 兜底。报错目前不提示候选列表（工具缺口，
> 见 open-items O-2026-08-16-3）。

| 任务                                             | 推荐入口                                                                                                                                                                                                                                                                    | 说明                                                                                                                                                                                                                                                      |
| ------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 恢复整体规划与方向（三项目协同路线）             | [`roadmap.md`](roadmap.md) + [`project-vision.md`](project-vision.md)                                                                                                                                                                                                       | 短/中/长期三层规划、里程碑、执行铁律与防丢失锚点；规划不代表当前已完成，落地情况看对应 PROGRESS/TASKS/open-items。                                                                                                                                       |
| 规划 3×3 自动求解/魔方求解准备                   | [`architecture/rubik-3x3-solver-preparation.md`](architecture/rubik-3x3-solver-preparation.md)                                                                                                                                                                              | 3×3 自动求解的游戏内可行性准备基线：状态模型、候选路线、存储方案与待验证约束；等待外部研究报告后更新。                                                                                                                                                   |
...
| 继续游戏实战编译器优化                           | [`architecture/compiler-practical-optimization-backlog.md`](architecture/compiler-practical-optimization-backlog.md)                                                                                                                                                        | 当前续作入口；登记已完成红绿回归、调用侧类型、诊断 provenance、实体类别、组件事件 lint、GIA 校验器、运动辅助层、生成/文档环境阻断和未归因游戏异常。不得跳过待验证项。                                                                                     |
| 制定逆向规则工具化任务                           | [`project-intelligence/reverse-engineered-rules-task-plan.md`](project-intelligence/reverse-engineered-rules-task-plan.md)                                                                                                                                                  | 汇总已解释但尚未工具化、工具不完整或尚未游戏验证的 GIL/GIA/组件/信号/Composite/客户端规则；每轮只推进一个最小测试点。                                                                                                                                     |
...
| 录入与沉淀 Project Knowledge                     | [`project-intelligence/knowledge-capture-canonical-flow.md`](project-intelligence/knowledge-capture-canonical-flow.md) + [`project-intelligence/knowledge-capture-common-errors.md`](project-intelligence/knowledge-capture-common-errors.md)                               | 从已提交变更开始，固定入口、bounded retrieval、串行 plan、精确 hash 审批和 apply 后验证；常见错误文档只记录可复用经验。                                                                                                                                   |
| 调查 GIL 整体字段树、资源/实体/图容器语义        | [`game-engine-knowledge/gil-structure-semantics.md`](game-engine-knowledge/gil-structure-semantics.md) + [`game-engine-knowledge/validation-workflow.md`](game-engine-knowledge/validation-workflow.md)                                                                     | 从根层做有界 wire 摘要，以编辑器单变化、不可变相邻快照和独立 Validator 逐章闭合；禁止按字段位置、相邻 ID 或 protobuf 默认值猜语义。                                                                                                                       |
...
| 精准读-改-写节点图/复合节点图逻辑                | [`architecture/gil-node-graph-edit.md`](architecture/gil-node-graph-edit.md) + 读侧工具 [`gia-tools-reference.md`](gia-tools-reference.md)                                                                                                                                    | `gsts assets:node-graphs read|patch`：位置/参数/连线/复合改名等已闭合操作（记录级局部替换 + 同构重放测试）；节点增删、复合接口结构变更、Variant 自动实例化等未闭合规则 fail closed。                                                                             |
...
| 图片素材/UI 图像（image editor 能力）            | [`image-editor.md`](image-editor.md)                                                                                                                                                                                                                                        | SceneDocument 场景模型、CSS/JSON/SVG 导入、JSON/CSS/SVG/GIA image 模式导出、`gsts image:*` CLI；自动回归通过，游戏加载核验待用户。                                                                                                                        |
...
整体 `.gil` 字段树与真实编辑器增量规则以 [`game-engine-knowledge/gil-structure-semantics.md`](game-engine-knowledge/gil-structure-semantics.md) 为当前 Authority；`.gia` 复合节点逆向使用以下入口：
...
[`composite-library/README.md`](composite-library/README.md)（19 类资源包总览 + 策展流程 `curation-process.md`）。
