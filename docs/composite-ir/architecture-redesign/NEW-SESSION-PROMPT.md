...
若 STATUS 未给出唯一工作包，或工作包范围/完成条件/验证记录与 Git 实质冲突，恢复报告中标明不一致并停止
等待我的决定。仅“STATUS 未写 commit SHA / 未写工作树 clean”不算冲突。只有已确定的当前工作包需要精确
历史证据时，才按 STATUS 的链接读取 work-packages/checkpoints 中对应章节。

修改前先向我提交协议规定的恢复报告，至少包含：
1. 当前分支、工作树和最近提交（来自 git；不要从 STATUS 抄 commit SHA）；
2. 当前 Phase 与唯一工作包；
3. 已确认事实和未确认假设；
4. 本轮拟读取与拟修改文件；
5. 实际验证命令与完成条件；
...
- Working tree 是否 clean，或每项变化是否都能由当前工作包解释（以 git status 为准，不查 STATUS 的 SHA）；
- 是否只有一个工作包；
- 是否把待验证假设误写成事实；
- 修改范围是否符合 phase；
- 是否触发需要用户决定的闸门。
