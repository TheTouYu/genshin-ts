# Composite Stage 3 Redesign 状态历史归档（2026-07-13）

> 状态：历史记录
> 来源：2026-07-13 精简前的 `STATUS.md` 快照；保留当时的工作包时间线与证据措辞
> 归档日期：2026-07-13
> 适用范围：查询已完成工作包的命令、候选路径、SHA-256 与失败过程；不作为新会话实时入口

> 当前状态请读上级 [`STATUS.md`](../STATUS.md)；当前 Phase 计划请读
...
当前工作包：P2-W17b 自动与用户编辑器核验通过，待用户审核（不提交）
最近完成代码工作包：P2-W17b — scalar same-type arithmetic shared identity resolution
分支起点：c5dfdd6 feat: add governed documentation search
工作树预期：P2-W17b Stage 3 源码、focused tests 与最小状态/Phase 2/验证矩阵文档更新待用户审核
ADR-006：Accepted = 方案 A（完整 vendor Graph materialization）
ADR-011：Accepted = 主图与 composite impl 的 ordinary 能力同源；后续以共享机制 + 更多真实可执行案例扩大验证覆盖
```
...
- **P2-W17a（提交 `b8449d1`）**：为 `addition`、`subtraction`、`multiplication`、`division` 的 int/float 同型 literal 与 ordinary connection target 建立 root、legacy impl 与 vendor-gated impl 观察基线。它确认 root 的 float concrete identity/schema 正确，而 impl float 回退为 int/generic identity；vendor gate 因而物化 int schema。该提交只记录失败契约，不改变生产 lowering，也不构成真实 GIA 或游戏行为结论。
- **P2-W17b（用户编辑器核验通过，待审核提交）**：上述四个 arithmetic family 的同型 int/float 输入现由 `resolveNodeIdentity()` 决定 `${node}__int` / `${node}__float` concrete identity，root adapter 与 impl 共同消费该决策。初版候选错误地丢弃 arithmetic 结果，形成纯数据孤岛；已改为可执行 fixture：主图 event 经 16 个 `Print String` 顺序到达 composite call，数据边为 arithmetic → DTC → Print；impl 由具名 `执行` InFlow 经 `compositePins` 进入首个 Print，再顺序经过其余 Print，composite 的四个 typed inputs 作为 Addition producer 数据源。focused fixture 已验证 root、legacy impl 和 `GSTS_STAGE3_VENDOR_IMPL_GRAPH=1` impl 的 literal/connection target identity/schema、数据连接、控制流和 composite InFlow route；resolved contract、DTC legacy/vendor、P2-W5 legacy/vendor 与 nested capture/outflow 回归通过。用户已确认修正候选在编辑器中通过；文件已从导出根目录归档到 `Beyond_Local_Export/真-测试通过/复合节点/P2W17b-scalar-arithmetic-vendor-shared-resolution.gia`，SHA-256 `929847e8078744dc6cd0356bfe726c1d91fcb5869ed1a4b2b397d3c18e4cc4a1`。未注入。异型输入、comparison、vec3、list/dict 仍未迁移。
...
- P2-W16 已覆盖当前 DTC 映射的 int/bool/float/entity/guid/vec3/faction 输入与 bool/int/float/str 输出；P2-W17b 仅补齐四则 arithmetic 的同型 int/float identity/schema 自动契约。这仍不证明 setter/getter、comparison、异型 arithmetic、vec3、list/dict 或其他 ordinary family 的 concrete variant 一致性。
...
- Signals/dynamic pin family 是否共用同一 resolution contract（ADR-008）；ADR-011 明确其目标能力与 root 同源，但专属 payload/dynamic pin/schema/wire 规则仍待用真实可执行案例验证。

## 用户确认的后续验证方向（ADR-011）

用户确认：主图可表达、可执行的 ordinary system node、API 调用和数据/控制流关系，目标上也应在 composite impl
中可表达、可执行。后续重构以共享 ordinary resolution/lowering/materialization 实现这一能力同源原则；复合只保留
synthetic call、capture、`compositePins`、inflow/outflow、wrapper 与布局等 boundary 职责。

该结论是目标架构与验证方向，不是“所有 API 已核验”的现状主张。后续按 family 以可执行 root + impl fixture、真实 GIA
和用户编辑器案例扩大覆盖；signal/dynamic pin 等特殊 family 若有专属规则，应接入共享 contract 下的专用 lowerer/normalization。
本方向不授权默认开启 vendor gate、删除 handwritten backend，或跳过真实/wire/游戏证据分层。
...
目标：为 `addition`、`subtraction`、`multiplication`、`division` 的 int/float 同型输入输出建立 root、legacy impl 与 vendor-gated impl 的观察基线；不接入 shared resolver 或改变生产 lowering。

自动观察：root 对四族的 int/float literal 与 ordinary connection target 都编码预期 generic/concrete ID（`200/200|201`、`202/202|203`、`204/204|205`、`206/206|207`）。legacy impl 的 float target 保留 float pin type，但 concrete ID 回退至 generic/int variant；vendor gate 因接收同一错误 concrete identity，进一步将 float target pin schema 物化为 int。该提交锁定后续 P2-W17b 的失败基线；未生成或复制游戏候选，未注入。

## P2-W17b 完成记录：scalar same-type arithmetic shared identity resolution

状态：实现、focused 自动回归与用户编辑器核验通过；待用户审核，不提交。

目标：仅将 `addition`、`subtraction`、`multiplication`、`division` 的同型 int/float concrete identity 迁移到 shared `resolveNodeIdentity()`，使 root、legacy impl 和 vendor-gated impl 使用同一 `${node}__int` / `${node}__float` 变种决策。

自动结果：四族 int/float 的 literal 与 ordinary connection target 均保持 root/impl 相同 generic/concrete ID（`200/200|201`、`202/202|203`、`204/204|205`、`206/206|207`）；legacy impl 与 vendor gate 的 float input/output pin 都为 float，不再将 float materialize 为 int schema。P2-W17a fixture 已由失败观察改为通过契约。

验证：`npm run build`、resolved-node contract、P2-W17b legacy/vendor fixture、P2-W16 legacy/vendor、P2-W5 legacy/vendor、nested capture/outflow 与 `git diff --check` 均通过。Node 26 对 `module.register()` 输出弃用警告，不影响退出码。

用户编辑器核验：通过。候选已从导出根目录移动至 `Beyond_Local_Export/真-测试通过/复合节点/P2W17b-scalar-arithmetic-vendor-shared-resolution.gia`；SHA-256：`929847e8078744dc6cd0356bfe726c1d91fcb5869ed1a4b2b397d3c18e4cc4a1`；未注入。

证据边界：当前为代码实现、自动回归和用户编辑器核验的 scoped evidence；未取得真实 GIA/wire 全等或注入证据。不得推广至异型 arithmetic、comparison、vec3、list/dict、capture/nested/boundary、布局、默认 gate 或 handwritten backend 删除。

## 待用户决策

P2-W17b 已通过用户编辑器核验，等待审核与提交指令。建议下一功能工作包先建立 comparison（同型输入、bool 输出）的独立观察基线；不得将本包结果推广至异型 arithmetic 或其他节点族。ADR-006=A 保持不变。现有证据仍不授权默认开启 gate、删除 handwritten backend，或将未覆盖的类型 wire/default、connection/capture 型 optional binding、`graphValues`、`affiliations` 纳入已完成范围。用户对后续 Stage 3 名称明确的候选 `.gia` 保持直接复制/覆盖 `Beyond_Local_Export` 根目录的授权；该授权不包含真实参考、归档、地图/注入目录、删除/清理或注入。
...
P2-W9 isolation 实现、自动回归、用户编辑器核验与文档更新已完成，提交为
`ebc478c refactor(stage3): isolate synthetic calls in vendor graphs`（Work package: P2-W9）。当前工作树预期 clean。
...
2. 检查分支、status 和最近提交；P2-W17b 审核前工作树应只含本轮源码、focused tests 和最小文档同步；
3. P2-W5~W12a、P2-W16、P2-W17a 已完成并提交；P2-W17b 待用户审核。下一功能工作包开始前读取
   `checkpoints/phase-2-vendor-embedding-evidence.md`、ADR-009、ADR-010、本文件 P2-W17b 事实和适用的官方节点规则查询；
