# Phase 2：共享 Vendor Ordinary-Node Lowering 历史归档（2026-07-13）

> 状态：历史记录
> 来源：2026-07-13 精简前的 Phase 2 文档快照；保留当时的切片、候选与证据措辞
> 归档日期：2026-07-13
> 适用范围：查询 P2-W1~P2-W17b 的详细命令、候选路径、SHA-256、失败与修正过程；不作为当前 Phase 计划

> 当前 Phase 计划请读上级
> [`phase-2-shared-vendor-node-lowering.md`](../phase-2-shared-vendor-node-lowering.md)，当前实时状态请读
> [`STATUS.md`](../STATUS.md)。
...
## P2-W17a：scalar arithmetic identity / vendor schema observation baseline（已提交 `b8449d1`）

观察 `addition`、`subtraction`、`multiplication`、`division` 的 int/float 同型输入输出，覆盖 root、legacy impl 和
`GSTS_STAGE3_VENDOR_IMPL_GRAPH=1` impl 的 literal/ordinary connection target。它锁定了迁移前失败基线：root 的 float target
使用预期 concrete ID（`201`、`203`、`205`、`207`）和 float schema；legacy impl 为 float target 写入 generic/int concrete ID
（`200`、`202`、`204`、`206`），vendor gate 因而 materialize 成 int schema。该提交未改变生产 lowering，也不构成真实 GIA 或游戏编辑器行为结论。

## P2-W17b：scalar same-type arithmetic shared identity resolution（用户编辑器核验通过，待审核提交）

仅将上述四族的同型 int/float identity 接入 `resolveNodeIdentity()`。root adapter 与 composite impl 同时消费 shared
`${node}__int` / `${node}__float` concrete variant；legacy impl 和 vendor gate 都因此对 float target 保留 float schema。focused
fixture 覆盖 literal 与 ordinary connection target，root/legacy/vendor 三个 scope 的 generic/concrete ID 与 input/output pin type
均转绿。

初版候选为纯数据孤岛，未能在编辑器核验有效数据流，已在同一工作包覆盖。当前候选把 arithmetic output 接入 DTC → Print，主图 event 顺序执行 16 个 Print 后进入 composite 的具名 `执行` InFlow；impl 的 `compositePins` 将该 InFlow 接入首个 Print，随后顺序执行其余 Print。composite 的四个 typed inputs 同时作为 Addition producer 的数据源，保留 ordinary producer → arithmetic connection 路径。

用户已在编辑器确认修正候选通过。文件已从导出根目录归档至
...
证据边界：这是 L1 shared resolution + L3 encoded fixture + L6 用户编辑器核验的 scoped evidence，不证明真实 GIA/wire 全等。异型 arithmetic、comparison、vec3、list/dict、capture/nested/boundary、布局、默认 gate 和 handwritten backend 删除不在本包范围。
...
5. ~~scalar same-type arithmetic~~（P2-W17a 观察基线已提交；P2-W17b int/float shared identity 已通过用户编辑器核验，待审核提交）；
6. comparison（输入同型、输出 bool，独立于 arithmetic；应先建观察基线）；
