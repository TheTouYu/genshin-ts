...
> 状态：进行中；P5-W1..P5-W4、P5-W6..P5-W9 完成；P5-W10 自动通过待用户核验
> 来源：目标架构设计 + Phase 4 checkpoint + P5-W1..W10 源码观察 + grilling 共享理解
> 最近校验：2026-07-16
...
- [x] P5-W9：pin-hole named adapter 整族共享收口（9 节点；矩阵 pin-hole green=9；
  ...
  已提交）。顺带最小共享补齐 assembly_list count/element 与 capture→pin-hole compositePins
  物理脚位；不宣称 special-arg 整族完成。
- [ ] P5-W10：special-arg named adapter 整族共享收口（5 节点；矩阵 special-arg green=5；
  自动 focused/matrix/inventory/sentinel 通过；待用户编辑器核验）。
