...
> 最近校验：2026-07-16
> 适用范围：真实 GIA 信号型复合逆向结论；方案 A 的目标地图已注册信号路径已完成用户游戏验证。
...
- gsts 如何从地图/信号注册表取得新信号三元组；
- gsts 是否应拒绝没有外部 SignalDef identity 的 `defineSignal`；
- SignalDef 的 `pinIndex` 在当前输出中的全局分配策略。
