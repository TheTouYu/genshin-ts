...
1. 提交本轮代码与文档。
2. 后续若继续布局优化，保持当前规则：导出前清理根目录旧 `.gia`，只保留当轮待测文件；通过后用 `mv` 归档。
3. 若未来出现类似数据区块/控制流贴近问题，优先在共享 `layout.ts` 中做局部区块避让，不要回到复合 impl 专用坐标补丁，也不要简单全局加大 root padding。

---

## 七、给下一位助手的一句话

> Round 15 已通过用户游戏内验证并归档。当前实现新增共享 `avoidExecLanesNearDataBlocks(...)`，R6-D 复合 impl 自动坐标显示第二条执行链后续节点由 y=730 下推到 y=1050；R6-C 自动回归未回到底部。五个 round15 GIA 已移动到 `真-测试通过/布局/`，导入根目录当前无 `.gia`；代码和文档待提交。
