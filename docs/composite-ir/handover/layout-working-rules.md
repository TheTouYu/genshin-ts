...
新 handover 统一使用 [handover-template.md](handover-template.md)。历史 handover 保持原样，不为套模板而回写。

1. 一轮只交接一个明确任务；正文以“下一轮目标”和“可用资源与执行边界”两个部分为主。
2. 每个 handover 必须把本文作为 P0 工作细节准则引用，并写明何时读、读哪些小节；引用不等于要求预先加载全文。
3. 文档资源按 P0/P1/P2 排序：工作准则、当前任务权威资料、歧义/架构升级入口。每项注明用途、触发条件和读取范围。
4. 只写下一轮可以直接使用的已验证基线、代码/测试入口、真实样本、易错点和推荐流程。
5. 已验证结论必须区分“游戏内通过”“真实 GIA 已验证”“自动验证通过”和“待验证”。
6. 不写会话时间线、完整日志/diff、subagent 状态、已解决且不影响下一轮的失败过程，也不重复通用 API 教程和本文已有规则。
7. 历史原因有保留价值时链接旧 handover；当前行为以源码、测试、真实 GIA 和当前权威文档为准。
8. 新增测试或导出文件时记录路径和验收用途；提交信息只在它是下一轮必要基线时保留。
