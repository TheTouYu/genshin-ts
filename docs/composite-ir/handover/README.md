...
| physics-r12 | [layout-handover-physics-motion-round-12.md](layout-handover-physics-motion-round-12.md)               | bool 输入参数 true/false 显示/选择问题 | 已完成：`CompositeDef.ParameterFlow.Type` 的 `EnumId { val: 1 }` 已修复并经自动回归和游戏内验证；复盘见 [r20-bool-enum-metadata.md](../retrospectives/r20-bool-enum-metadata.md)。 |
| physics-r13 | [layout-handover-physics-motion-round-13.md](layout-handover-physics-motion-round-13.md)               | 复刻 `计算物理运动状态` 并接入 `更新v、w` | 已完成阶段性接入：vec3 literal `(0,1,0)` 游戏内通过；`v停止` 和完整真实结构仍待继续核验。 |
| physics-r14 | [layout-handover-physics-motion-round-14.md](layout-handover-physics-motion-round-14.md)               | 优化 `v停止`/嵌套复合结构与 literal 类型族编码 | 当前推荐：逐字段对齐真实 `与`、`can fly`、`v停止`/`停止` 和 compositePins；扩展 int/float/bool/str/vec3 等 literal 自动回归。 |
| dtc-boundary-pins | [dtc-boundary-physical-pins.md](dtc-boundary-physical-pins.md)                             | DTC 边界物理引脚修复：capture 输入不自动等于“不需要物理 pin” | 已完成：DTC 节点若被 compositePins 指向必须保留物理 InParam 并独立生成 OutParam；自动回归 + 游戏内通过。详见 [../retrospectives/r20-bool-enum-metadata.md#附录-1后续发现dtc-边界物理引脚缺失](../retrospectives/r20-bool-enum-metadata.md#附录-1后续发现dtc-边界物理引脚缺失)。 |
