...
| v2-r6       | [v2-composite-validation-round-6.md](v2-composite-validation-round-6.md)                           | all-types 自动生成覆盖与 list 类型缺口 | core profile 已可导入打开；collections 暴露 composite impl 中 `vec3_list/config_id_list/prefab_id_list/faction_list` 等 pin.type 编码为 `0` 的严重问题。下一轮先修 `composite.ts` 类型映射并跑 `list-type-ops-smoke` TDD。 |
