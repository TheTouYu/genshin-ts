...
| physics-r4  | [layout-handover-physics-motion-round-4.md](layout-handover-physics-motion-round-4.md)                 | InFlow fan-out、literal pin、impl 布局修复与新反馈核验 | 已修复并注入；新反馈确认：`get_custom_variable` pin index 被压缩、`asType('float')` 输出 concrete 类型缺失、`S`/`D` 应保留但不接控制流。 |
