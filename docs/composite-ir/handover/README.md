...
| physics-r13 | [layout-handover-physics-motion-round-13.md](layout-handover-physics-motion-round-13.md)               | 复刻 `计算物理运动状态` 并接入 `更新v、w` | 当前推荐：复刻真实 11 节点复合，先替换 `更新v、w` 的 `接触地面=false` 阶段性输入；不处理 Update、信号系统和 54 节点物理运动控制器。 |
