...
- 若此提示显示为“警告”而非“错误”，通常表示项目的 TypeScript 插件已生效（已将 `bigint` 视作可索引），可按需禁用 `gsts/require-bigint-index-wrapper` 规则。
...
- 同时使用 `@types/node` 时，GSTS 的类型入口会优先加载 Node 声明，未标注的 Timer callback 参数仍会推断为节点图事件和 `ServerExecutionFlowFunctions`。
- `gsts.timers.setTimeout/setInterval` 是不与 Node 全局 API 同名的等价入口；需要避免全局名称冲突或明确 API 来源时优先使用它。清理对应句柄使用 `gsts.timers.clearTimeout/clearInterval`。
