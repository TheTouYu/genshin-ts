...
该操作要求 ID、名称和实例到定义的引用全部匹配，否则失败关闭。`components` 同步更新定义与指定实例，已有跟随运动器槽时替换而不叠加；`scale` 只更新指定场景实例的缩放，保持位置、旋转和元件定义不变。它不会创建元件或辅助 ID。CLI 仍默认 preview，`--output` 生成离线候选，`--write` 自动备份后写回。当前原地更新能力已有自动回归和离线真实地图候选验证；具体写回与游戏表现必须另行确认，不能由自动测试替代。
