...
- If this is shown as a warning (not an error), the project TypeScript plugin is usually active and already treats `bigint` as a valid index value; you may disable `gsts/require-bigint-index-wrapper` if preferred.
...
- When using `@types/node`, the GSTS type entry loads Node declarations first, so unannotated Timer callback parameters still infer the graph event and `ServerExecutionFlowFunctions` types.
- `gsts.timers.setTimeout/setInterval` are equivalent APIs that do not share names with Node globals. Prefer them when avoiding global-name conflicts or making the API source explicit; clear their handles with `gsts.timers.clearTimeout/clearInterval`.
