...
| L1 普通主图 vs composite impl | 同一 `f.*` 调用在普通主图和 impl graph 中的 node ID、pin type、literal、connect 是否一致 | `system-node-reuse-smoke.ts` + `compare-system-node-reuse.ts`（默认诊断模式，`--strict` 表示严格同构验收；当前 L1 样本已严格通过） |
