...
import { convertPrefabStatic, createCustomPrefab, createStaticPrefabInstance, updatePrefabDefinition } from './gil_prefabs.js'
...
function usage(exitCode = 1): never {
  const output = [
    ...
    '       gsts assets:prefabs update --id <definitionId> [--force]',
    '',
    'Options:',
    ...
    '  update:',
    '  --force                 force-overwrite entity component slots (drop per-entity edits)',
...
function parseArgs(argv: readonly string[]) {
  let command: 'create' | 'convert' | 'update' = 'create'
  ...
  if (command === 'create') {
    ...
    if (newId === undefined) throw new Error('[error] update requires --id <definitionId>')
  }
  if (write && outputPath) throw new Error('[error] --write and --output are mutually exclusive')
  return { command, baseId, newId, name, gilPath, outputPath, write, format, position, isStatic, convertStatic, force }
}
...
async function execute(argv: readonly string[], cfg: GstsConfig | undefined) {
  ...
  if (args.command === 'update') {
    const result = updatePrefabDefinition(src, { id: args.newId!, force: args.force })
    const summary: Record<string, unknown> = {
      schemaVersion: 1,
      kind: 'prefabs-update',
      sourceSha256: sourceHash,
    ...
    log(
      `prefabId=${result.id} force=${args.force} entitiesUpdated=${result.entitiesUpdated} entitiesPreserved=${result.entitiesPreserved}`
    )
    summary.candidateSha256 = sha256(result.bytes)
    log(`candidateSha256=${sha256(result.bytes)}`)
    if (args.outputPath) {
