...
| 批次 | 资源包 | 复刻节点数 | 比对结果 |
|---|---|---|---|
| batch1 | 变量运算包（8）+ 逻辑运算包（6） | 14 | **14/14 PASS**（vs 社区资源包） |
| batch2 | 随机工具包（2）+ 矩阵运算包（4） | 6 | **4/4 PASS + 2 复刻差异**（vs 社区资源包） |
| batch3 | 数学/几何（4）+ 长列表（1） | 5 | **独立编译通过**（从 rubik 项目提取，脱离魔方状态可复用） |

**总计：25 个复合节点。20 个与社区资源包比对（18 全一致 + 2 设计差异）；5 个从魔方项目提取验证独立可复用。**
