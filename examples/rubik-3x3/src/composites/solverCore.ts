...
// exec：把一个 move code 追加到 solveBuf（face 转 cnt 次 + 更新 solveLen）
  ...
  build: (_i, f) => {
    ...
    f.connectOutFlow(br, 1, () => {
      ...
      f.doubleBranch(f.greaterThan(mLen, 0n), () => {
        f.callComposite(solverAppendCode, { code: mC0 })
        f.doubleBranch(f.greaterThan(mLen, 1n), () => {
          f.callComposite(solverAppendCode, { code: mC1 })
          f.doubleBranch(f.greaterThan(mLen, 2n), () => {
            f.callComposite(solverAppendCode, { code: mC2 })
          }, () => {})
        }, () => {})
      }, () => {})
    })
