...
- 2026-08-31（第二阶段 R0：参数映射表 + 差距清单，未动代码未注入，待用户确认）：
  ...
  - **验收锚点**：B1b 2m 落体回高 0.9418m（球底口径）/ B4a 侧旋 20m 横偏 +3.6848m / B3a 8m/s 滚停 22.91m，含游戏内复现协议与轴映射（sim +Z↔game ±X）。
  - **用户已确认（2026-08-31）**：半径方案 A（实体 0.25 + FIFA 气动/反弹）；R1 按范围进入；ROLL_DECEL→1.0（sim 基准）；L2=#1 TOI 能量反弹 + #1b 门柱 swept（#2/#3 暂缓）；C1 带球族随 R3 一并；STOP_SPEED 留 R3 实测。
