// @gsts:entry
// 灯阵玩法图 L5（v6 内联，由 tools/gen-levels.mjs 生成，勿手改）
// 阵形 3×3，胜利 9 灯，预置掩码 325
...
const graph = g
    ...
    const ixInit = f.roundToIntegerOperation(f.division(f.subtraction(loc.x, 2.5), 2.5), RoundingMode.RoundToNearest);
    const izInit = f.roundToIntegerOperation(f.division(f.subtraction(loc.z, 2.5), 2.5), RoundingMode.RoundToNearest);
    const indexInit = f.addition(f.multiplication(izInit, 3n), ixInit);
    const pow2 = f.exponentiation(2n, indexInit);
    const shifted = f.division(325n, pow2);
    ...
    f.doubleBranch(f.lessThanOrEqualTo(dist, 0.1), () => {
        ...
        f.doubleBranch(f.equal(after, 9n), () => {
            const gsts = globalThis.gsts;
            f.printString('lamp-win');
            f.sendSignal(LampSig.level_clear, 5);
    ...
    const self = f.getSelfEntity();
    const head = f.getCustomVariable(self, new str('head')).asType('entity');
    f.doubleBranch(f.equal(evt.params.level, 5), () => {
        const gsts = globalThis.gsts;
        const loc = f.getEntityLocationAndRotation(self).location;
        const ix = f.roundToIntegerOperation(f.division(f.subtraction(loc.x, 2.5), 2.5), RoundingMode.RoundToNearest);
        const iz = f.roundToIntegerOperation(f.division(f.subtraction(loc.z, 2.5), 2.5), RoundingMode.RoundToNearest);
