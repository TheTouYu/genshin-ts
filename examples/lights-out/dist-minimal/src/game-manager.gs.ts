...
import { LEVELS, waveTotalSeconds } from "../../src/levels.js";
...
const buildLevelBlock = (f: any, self: any, cfg: typeof LEVELS[number]) => {
    const n = cfg.sizeX * cfg.sizeZ;
    for (let k = 0n; k < n; k++) {
        const ix = f.moduloOperation(k, cfg.sizeX);
        const iz = f.division(k, cfg.sizeX);
        const x = f.addition(cfg.originX, f.multiplication(f.dataTypeConversion(ix, 'float'), cfg.spacing));
        const z = f.addition(cfg.originZ, f.multiplication(f.dataTypeConversion(iz, 'float'), cfg.spacing));
        f.createPrefab(cfg.prefabId, f.create3dVector(x, 0, z), f.create3dVector(0, 0, 0), self, false, 0, new listLiteral('int'));
    }
...
const graph = g
    ...
    f.doubleBranch(f.equal(evt.tabId, 1), () => {
        const gsts = globalThis.gsts;
        ...
        const gsts = globalThis.gsts;
        f.doubleBranch(f.equal(evt.tabId, 2), () => {
            const gsts = globalThis.gsts;
            // 重开本关：清空当前所有灯柱再重建第一关
            ...
            f.startTimer(self, 'restart', false, gsts.f.assemblyList([0.5]));
        }, () => {
            const gsts = globalThis.gsts;
            f.doubleBranch(f.equal(evt.tabId, 3), () => {
                const gsts = globalThis.gsts;
                ...
                f.sendSignal(M.lamp_wipe, 5);
                f.sendSignal(M.lamp_wipe, 6);
                f.startTimer(self, 'back', false, gsts.f.assemblyList([0.5]));
            }, () => {
                const gsts = globalThis.gsts;
                f.doubleBranch(f.equal(evt.tabId, 4), () => {
                    const gsts = globalThis.gsts;
    ...
    // 控制台旋转庆祝 + 广播波浪信号
    f.addUniformBasicRotationBasedMotionDevice(self, 'celebrate', 2.0, 180, gsts.f.assemblyList([0, 1, 0]));
    f.printString('console-spin');
    f.sendSignal(M.win_wave, level);
    // 按关卡选择波浪时长（常量表已离线计算为 number）
    f.doubleBranch(f.equal(level, 1), () => {
        const gsts = globalThis.gsts;
        f.startTimer(self, 'wipe1', false, gsts.f.assemblyList([gsts.f.addition(1.15, 0.2)]));
        f.startTimer(self, 'build2', false, gsts.f.assemblyList([gsts.f.addition(1.15, 0.3)]));
    }, () => {
        const gsts = globalThis.gsts;
        f.doubleBranch(f.equal(level, 2), () => {
            const gsts = globalThis.gsts;
            f.startTimer(self, 'wipe2', false, gsts.f.assemblyList([gsts.f.addition(1.3, 0.2)]));
            f.startTimer(self, 'build3', false, gsts.f.assemblyList([gsts.f.addition(1.3, 0.3)]));
        }, () => {
            const gsts = globalThis.gsts;
            f.doubleBranch(f.equal(level, 3), () => {
                const gsts = globalThis.gsts;
                f.startTimer(self, 'wipe3', false, gsts.f.assemblyList([gsts.f.addition(1.45, 0.2)]));
                f.startTimer(self, 'build4', false, gsts.f.assemblyList([gsts.f.addition(1.45, 0.3)]));
            }, () => {
                const gsts = globalThis.gsts;
                f.doubleBranch(f.equal(level, 4), () => {
                    const gsts = globalThis.gsts;
                    f.startTimer(self, 'wipe4', false, gsts.f.assemblyList([gsts.f.addition(1.75, 0.2)]));
                    f.startTimer(self, 'build5', false, gsts.f.assemblyList([gsts.f.addition(1.75, 0.3)]));
                }, () => {
                    const gsts = globalThis.gsts;
                    f.doubleBranch(f.equal(level, 5), () => {
                        const gsts = globalThis.gsts;
                        f.startTimer(self, 'wipe5', false, gsts.f.assemblyList([gsts.f.addition(2.2, 0.2)]));
                        f.startTimer(self, 'build6', false, gsts.f.assemblyList([gsts.f.addition(2.2, 0.3)]));
                    }, () => {
                        const gsts = globalThis.gsts;
                        // level 6：触发 win30，wipe 由波浪本身（已是 win_wave）触发；
                        // 不再创建 build7
                        f.startTimer(self, 'wipe6', false, gsts.f.assemblyList([gsts.f.addition(2.2, 0.2)]));
                    });
                });
            });
        });
    ...
    const gsts = globalThis.gsts;
    const self = f.getSelfEntity();
    f.doubleBranch(f.equal(evt.timerName, new str('build2')), () => {
        const gsts = globalThis.gsts;
        f.printString('level2-building');
        buildLevelBlock(f, self, L2);
    }, () => {
        const gsts = globalThis.gsts;
        f.doubleBranch(f.equal(evt.timerName, new str('build3')), () => {
            const gsts = globalThis.gsts;
            f.printString('level3-building');
            buildLevelBlock(f, self, L3);
        }, () => {
            const gsts = globalThis.gsts;
            f.doubleBranch(f.equal(evt.timerName, new str('build4')), () => {
                const gsts = globalThis.gsts;
                f.printString('level4-building');
                buildLevelBlock(f, self, L4);
            }, () => {
                const gsts = globalThis.gsts;
                f.doubleBranch(f.equal(evt.timerName, new str('build5')), () => {
                    const gsts = globalThis.gsts;
                    f.printString('level5-building');
                    buildLevelBlock(f, self, L5);
                }, () => {
                    const gsts = globalThis.gsts;
                    f.doubleBranch(f.equal(evt.timerName, new str('build6')), () => {
                        const gsts = globalThis.gsts;
                        f.printString('level6-building');
                        buildLevelBlock(f, self, L6);
                    }, () => {
                        const gsts = globalThis.gsts;
                        f.doubleBranch(f.equal(evt.timerName, new str('wipe1')), () => {
                            const gsts = globalThis.gsts;
                            f.sendSignal(M.lamp_wipe, 1);
                            f.printString('wipe1-sent');
                        }, () => {
                            const gsts = globalThis.gsts;
                            f.doubleBranch(f.equal(evt.timerName, new str('wipe2')), () => {
                                const gsts = globalThis.gsts;
                                f.sendSignal(M.lamp_wipe, 2);
                                f.printString('wipe2-sent');
                            }, () => {
                                const gsts = globalThis.gsts;
                                f.doubleBranch(f.equal(evt.timerName, new str('wipe3')), () => {
                                    const gsts = globalThis.gsts;
                                    f.sendSignal(M.lamp_wipe, 3);
                                    f.printString('wipe3-sent');
                                }, () => {
                                    const gsts = globalThis.gsts;
                                    f.doubleBranch(f.equal(evt.timerName, new str('wipe4')), () => {
                                        const gsts = globalThis.gsts;
                                        f.sendSignal(M.lamp_wipe, 4);
                                        f.printString('wipe4-sent');
                                    }, () => {
                                        const gsts = globalThis.gsts;
                                        f.doubleBranch(f.equal(evt.timerName, new str('wipe5')), () => {
                                            const gsts = globalThis.gsts;
                                            f.sendSignal(M.lamp_wipe, 5);
                                            f.printString('wipe5-sent');
                                        }, () => {
                                            const gsts = globalThis.gsts;
                                            f.doubleBranch(f.equal(evt.timerName, new str('wipe6')), () => {
                                                const gsts = globalThis.gsts;
                                                f.sendSignal(M.lamp_wipe, 6);
                                                f.printString('wipe6-sent');
                                            }, () => {
                                                const gsts = globalThis.gsts;
                                                f.doubleBranch(f.equal(evt.timerName, new str('win30')), () => {
                                                    const gsts = globalThis.gsts;
    ...
    const gsts = globalThis.gsts;
    const self = f.getSelfEntity();
    f.doubleBranch(f.equal(evt.timerName, new str('restart')), () => {
        const gsts = globalThis.gsts;
        f.printString('restart-building');
        buildLevelBlock(f, self, L1);
    }, () => {
        const gsts = globalThis.gsts;
        f.doubleBranch(f.equal(evt.timerName, new str('back')), () => {
            const gsts = globalThis.gsts;
