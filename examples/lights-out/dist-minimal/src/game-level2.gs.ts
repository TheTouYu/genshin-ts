...
const graph = g
    ...
    const ixInit = f.roundToIntegerOperation(f.division(f.subtraction(loc.x, 5), 2.5), RoundingMode.RoundToNearest);
    ...
    f.doubleBranch(f.lessThanOrEqualTo(dist, 0.1), () => {
        ...
        f.doubleBranch(f.equal(after, 3n), () => {
            const gsts = globalThis.gsts;
    ...
    const self = f.getSelfEntity();
    const head = f.getCustomVariable(self, new str('head')).asType('entity');
    f.doubleBranch(f.equal(evt.params.level, 2), () => {
        const gsts = globalThis.gsts;
        const loc = f.getEntityLocationAndRotation(self).location;
        const ix = f.roundToIntegerOperation(f.division(f.subtraction(loc.x, 5), 2.5), RoundingMode.RoundToNearest);
