...
- 调查：用 debug-log-investigator 解析最新会话日志，逐轮追踪 orbit 段位置读取/速度计算/运动器参数，定位漂移来源
- 候选根因（待日志证实，不猜）：①公转公式用运行时位置（误差累积）②自旋轴语义（世界轴 vs 局部轴）与公转叠加 ③5 段折线端点与自旋 90° 的合成误差 ④转动结束后位置未对齐网格
- 修复：按日志结论定（如公式改为按初始位置计算、或转动后修正位置）
