...
### P4-3 CLI 组件移除能力 + 组件操作验证——🟡 工作①②完成（待游戏核验+工作③矩阵）
...
- 工作①（✅ 完成 2026-08-16）：`GstsStaticPrefabUpdate.removeComponents`（类型码列表，定义槽 8 + 实例槽 7 双写移除；
  ...
  + `npm run quicktest` 绿；文档 zh/en config.md 同步。提交 1ff4d6a。
- 工作②（✅ 完成 2026-08-16）：移除 1077936135 的 12/13 已写回。探针确认仅该角块多出 12/13（其余 7 块标准 7 槽）；
  候选 diff 只动该记录（25 图+24 复合零改动）；写回后 SHA 4a2368c2（与候选一致），备份
  `.gsts/backups/1073741882.gil.2026-08-15T05-29-23-697Z.bak`，Temp 已同步；回读探针 def+inst 均为标准
  7 槽（18/1/3/19/6/14/4）。证据：`~/genshin-ts-evidence/p43-remove-components/`。
  ⚠️ **待用户游戏核验**：重新加载地图/游戏后角块 UFL 正常（移除 12/13 不影响运动器；编辑器旧内存可能覆盖写回）
- 工作③：逐组件验证矩阵（添加→回读→游戏核验→移除）——待安排（验证地图上做，不碰魔方地图）
...
## 基础设施轮（2026-08-17 新增，编译验证通过，未注入）

### I1 注入 + 回读核验（下一步）
- 用 verify-injection / 正式注入通道把新 game.gia 注入图 1073741825（备份 → 注入 → dump 回读节点/连线与 IR 一致）
- 验收：回读结构与 IR 一致；游戏加载正常
- 证据层：CLI 回读 + 用户游戏核验

### I2 游戏核验清单（用户确认后执行）
- ① 手动转动 R/L/U/D/F/B 行为与 v5.5 一致（无回归）
- ② 打乱（tab7）：20 步随机播放、输入锁、播放完停住
- ③ 手动复原后自动胜利：拼好 → 结算成功（日志 rubik-solved-win）
- ④ 复原（tab8）：占位提示（rubik-solve-placeholder）；已还原时直接结算
- ⑤ 重置（tab9）：角块销毁重建、逻辑/流程复位、可重新游玩
- ⑥ 日志验证：Beyond_Debug_Log 逐帧核对逻辑状态读写/队列推进
- 证据层：用户游戏核验 + 日志逐帧

### I3 2×2 自动复原实装（下一轮主任务）
- 求解器（宏库/微型 PDB，参考 web-prototype first-layer/second-layer/cfop + 3×3 准备基线）→ 产出 queue+qLen → 接入 gstsSolve
- 状态编码与原型 cube.js 对齐（twist 已一致）
- 预算：总节点 419 + 求解器，控制在 2000 内；单列表/字典规模按 500 约束核算
