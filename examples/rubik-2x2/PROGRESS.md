...
- [ ] **关卡实体挂载玩法图（阻碍项）**：关卡实体是游戏自动创建、不在 root5 场景实体列表中的默认实体；`whenEntityIsCreated` 需挂载到它才能触发。规则未闭环（挂载 GIL 结构未知），CLI `assets:mounts` 尚不支持。方法：用户在编辑器为关卡实体挂载最小图 → 保存相邻快照 → 拆分解析比对 → CLI 支持。**先于 P4 第二步完成。**
...
- [ ] 运行时动态创建元件实体的节点图能力（P4，依赖关卡实体挂载阻碍项）
