...
### Round 2h（自定义造物绑定节点图，绑定位置第三形态）

- 用户操作：给自定义造物技能挂载对应节点图（1082130435=20008 造物技能）；AFTER hash `e0b4fefa…`，size 801685。
- 归因（v11→v12）：绑定 = body `4[1].78.1` 追加 `f4={1:图ID, 2:1073741825}`（**第三种绑定位置**，区别于普通技能的 body f5、自定义技能的 event track f4）。
- 同构重放：从 v11 追加 f4 绑定 → root15 与 v12 **逐字节一致**。
- 联动：root 20（造物模型）随绑定增长 1970→2955B（模型↔图联动，open-item）。
- 技能↔图类型映射补全：f2=28→20008 造物技能（f2=36→20010 角色操控，f2=6→20002 角色技能）。
