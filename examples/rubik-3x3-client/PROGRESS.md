...
- 规则状态：**规则 2 CONFIRMED**（技能配置创建 + 节点图引用 + 事件轨道结构）；已更新知识。
- 未闭合（open-items）：`f5.f2=1073741825` 语义；事件轨道 `f45.f2` 计数与打点结构语义；`4[1].30.1.1.4` 普通/瞬发是否还有更多取值。
- 下一步：CLI 设计（`assets:node-graphs create --type 20010` 扩展 + 技能配置命令形态）。
