...
### Round 2i（切换释放类型自动丢挂载，引擎正常行为）

- 用户操作：把自定义造物技能的释放类型从瞬发改普通；AFTER hash `55e44e96…`，size 800830。
- 归因（v12→v13）：
  - 释放类型字段：自定义造物的瞬发/普通 = `4[1].78.1.1.f2`（瞬发=缺省+node引用10007001；普通=`f2:2`+node引用10005001）。
  - **切换类型自动丢挂载**：body 绑定 `f4` 消失；事件轨道从简化 25B 恢复默认 107B；root 20 从 2955B 回退 1970B。
- 规则：切换释放类型会清空节点图挂载（引擎正常，CLI 生成时需注意「先定类型再绑图」）。

### Round 2j（造物模型切换 + 语义澄清）

- 用户操作：造物模型从「遗迹守卫」换成「打手丘丘人」；AFTER hash `eceea3ca…`，size 800830（与 v13 同大小）。
- 归因（v13→v14，仅 root15/16 + root46，**root20 不变**）：模型引用 = `4[1].78.1.2.1`，10005001(遗迹守卫)→10007001(打手丘丘人)，仅此一字节变。
- 语义澄清（用户）：**模型引用是造物技能特有**——角色技能/普通技能的模型就是角色本身；造物技能给造物用，才带造物模型选择。
- 模板语义：f2=36/6 = 角色用技能；f2=28 = 造物用技能（带模型引用 + root 20 造物模型容器）。

### Round 3a~3e（三模板 × 两释放类型组合缺口全闭合）

> 证据：`…/2026-08-29-client-graph-skillconfig/raw/after-36-normal.gil` 起 5 个新快照（见 manifest Round 3a~3e）。

- **3a（36 瞬发→普通）**：丢挂载三模板全证（28/36/6）；记录恢复 v3 创建默认形态（512B 逐字节一致）；45.2 62→61。
- **3b（36+普通 绑图）**：事件轨道 45.1 保留默认条目 + 单打点 f3{2:3001 无f9} + f4{图ID,1073741825,268435457}；45.2 不变。
- **3c（6 普通→瞬发）**：45 默认条目×2+打点+绑定全清，45.1 剩 {9,10}，45.2 21→2、45.16 移除。
- **3d（6+瞬发 绑图）**：body 30.1.5 f5（与 36+瞬发同构）；45.2 绑定不改 → **45.2 = 模板×释放类型形态值**（36:61/62、6:21/2、28:51/52）。
- **3e（28+普通 绑图）**：事件轨道 45.1 打点 f3{2:7001001 带f9}（用户手动加的造物动画）+ f4；root20 不联动（联动仅 28+瞬发 body 78.1.4）；**用户确认：28+普通 CLI 不模拟仅列出**。
- 核心规律：**绑定位置由释放类型决定**（瞬发=body f5 / 普通=事件轨道打点）；打点资源模板相关（36:3001、6:1001/1004/1007、28:7001001）。
- 同构重放：3a~3e 每轮 root6/10/15/16 与 after 逐字节一致（5/5 通过）。
- 下一步：PKC 落盘（组合矩阵 + 45.2 形态表 + 绑定位置规律）+ CLI 实现（支持矩阵按上表：36/6 全组合生成，28 仅列出）。

### Round 4（CLI 落地 + 真实写回 + 编辑器识别核验通过）

- **PKC 落盘**：bundle bnd_f8e9a327（3 claims：客户端图创建 / 技能配置创建 / 技能配置绑定）已 apply 并提交；
  bundle bnd_efba42（C1 revise：客户端图 ID 段起始值闭合）已 apply 并提交。
- **CLI**（已提交 532c321 / 8a82400）：
  - `assets:node-graphs create --type <数字|中文名>`：20000/20001/20002/20006/20008/20009/20010；
    客户端图自动节点按类型模板；folder 追加到类型级 folderId 常量记录；20003/20004/20005/20007 fail closed。
  - `assets:skill-config create|list`：normal/custom × 瞬发/普通 + 绑定（瞬发=body f5 轨道点 1825+7n；
    普通=事件轨道打点 36:3001/6:1001..1007 限 1 图）；28 模板 fail closed（root20）；45.2 形态值表。
  - 修复：root10 新图插入位置 = 最后一张既有图后（混合地图复合定义错位 bug）。
  - 客户端图 ID 自动分配：段起始 **1082130433（用户初始地图实测）**，段内 max+1。
- **回归**：tests/gil_client_graph_skillconfig_test.ts 17 项同构重放逐字节一致（快照 1073741914 差分轮）。
- **真实写回（用户确认安全门后）**：1073741914 写入 20010 图 1082130441「测试-CLI建图」+
  技能配置 1228931074「测试-CLI技能」（36+瞬发+f5 绑定）；写后 SHA == 候选 SHA；
  备份 .gsts/backups/（new-graph/new-skill-config 各 1）。
- **编辑器识别验证：用户核验通过**（测试资产可识别）。
- 遗留：跨类型 ID 复用规则 INSUFFICIENT；打点资源 3001/7001001 语义未闭合；28 模板创建/绑定 CLI 未复刻。

### Round 5（28 造物模板 CLI 支持 + 全组合真实写回 + 编辑器验收通过）

- **CLI 28 模板支持**（91b1048）：创建/绑定用 v11/v12 快照字节模板（99B/113B）；**固定造物模型 = 10005001 遗迹守卫**
  （用户确认：造物技能固定一模型为引擎合法行为，普通/瞬发共用）；root20 三态处理（空占位/1970B 创建态/2955B 绑定态，
  其它 fail closed）；78.1.4 绑定图 ID 参数化；list 回读修复（78.1.4 在 78.1 内解析）。
- **组合矩阵全覆盖**（真实地图 8 个技能配置）：36×2（瞬发/普通）+ 6×2（瞬发/普通）+ 28×2（瞬发 CLI 生成/普通编辑器样本）。
- 真实写回批次：1098907650/1098907651（6 模板）、1228931075（36+普通）、1191182338（28+瞬发+root20 联动）；
  写后 SHA == 候选，全部独立回读通过。
- **编辑器识别验证：全部通过**（用户核验，2026-08-29）。
- PKC：bnd_ae7c7e87（C2/C3 revise：28 模板支持 + 固定模型 + root20 三态）已 apply。
- 注入验证：clientProbe.gia → 1082130441（nodeCount=5，_GSTS_clientProbe，读图核验通过）；
  游戏实测（服务端主动调用技能触发）待做。

---

## 性能优化任务 Round 0（2026-08-31，客户端计算降服务端负载）

> 任务书：把适合客户端图承担的计算/调度挪到客户端，降低服务端 perf 负载指标。
> 产出文档：`MIGRATION-PROPOSAL.md`（基线数据 + 官方拆解 + 候选清单 + 第 1 轮提案）。

### R0.1 K-01 状态确认

- `npm run test:client-transform` exit 0 全绿（assert + 9 smoke 全 [ok]）——08-29 修复仍在位，
  本轮零改动直接通过。

### R0.2 服务端负载基线（历史日志，无需用户现跑）

- **有效基线 = 2960**（08-28 自动求解会话 ~500 转动/387s，在 1073741899 上跑；图族与
  1914 完全一致）。2956/2958 同族可作重复样本。2979/2980 = 官方客户端优化版参照
  （2980 手动 5 操作完整还原：D/L/x/D/L）。
- 图级分布：visual 19.2MB(50%) + turn 15.3MB(40%) + solver 族 <2%——**求解器不是矛盾**。
- 单次面转：我们 ~5450 负载 vs 官方 ~2500（**-54% 空间**）；整转 ~16000 vs 6195（-61%）。
- 峰值秒：8754 负载/6297 帧（2960 sec883）。
- 驱动方式违例：7 图全 When Timer；错峰定时器 0.01~0.09s 九槽 + orbit2 0.16~0.24s
  （0.06s/触发禁止长期运行的铁律违例）；整转间隔被迫 7.18/6.6/10.45s。

### R0.3 官方范例拆解（读图证据，修正一处旧认知）

- 客户端图 1082130436（191 节点）干：指令解析（多分支组合表）+还原检测+层归属判断
  （遍历 26 块读"位置"+6 比较）+旋转计算（3D Vector Rotation×2+取整 807/801）+busy 锁
  （addUnitStatus）+信号回传（6 参数含异常通道 n=116）。
- 服务端「魔方块-旋转」30 节点小图挂每块实体：监听→写回 3 变量（Get→3D Rotation→取整→Set）
  →**Target-Oriented 单设备/块**（时长=信号）→动画停 Remove Unit Status。
- **修正旧认知**：官方服务端确实重算每块位置（不是客户端传结果回来）——信号只传
  时长/角度/是否还原/整体向量。服务端残留负载 = 每块写回链 + 1 设备。

### R0.4 候选清单（详见 MIGRATION-PROPOSAL.md）

- A 旋转数学迁客户端（主线，-54%）；B 动画事件驱动化+单设备化（纯服务端，先行铺路）；
  C visual_order 重排待查；D solver 不迁；E busy 锁跟随 A。
- **第 1 轮提案：B 先行**（等用户确认方向）。

### R0.5 待用户决策

1. 第 1 轮走 B（纯服务端动画改造）还是 A 最小切片（客户端算单块）？
2. 基线是否需要用户在 1073741914 上现跑一轮同操作序列（更严格的同口径对比锚点）？
   历史基线 2960 已可用，但那是 1073741899 上的自动求解会话。
