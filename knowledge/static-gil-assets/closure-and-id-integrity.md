...
编辑器“空物体/空模型” = root5 场景实体记录：f2={f1:10005018, f2:1}（资源 ID 直接作定义引用，无 root4 定义配对）、f8=10005018、f5 槽 11 个（含 t=40 挂接槽）、f6 槽 14 个（transform + 默认色材质 f3=0xFFFFFFFF）、f7 组件 6 个；可作无可见资源宿主挂装饰物。宿主 scale 建议缩至 0.1~0.01 防空模型 1×1×1 体积重叠闪烁（2026-08-12 结论，原“保持 [1,1,1]”建议已废弃）。创建新实体时 root6 大记录（组ID=3）f3.f5 列表末尾追加 {f1:200, f2:新实体ID}；root46 等长保存副作用；无 root22 新条目。
...
<!-- CLAIM:START clm_D68D1E5F559964FD1512BB0E6A -->

### 切换静态/动态的 wire 规律（删/加组件槽 + 名字槽标记）

动态→静态切换 = 删组件槽（定义 f8、实例 f7、引用实体 f7）+ 名字槽 f11 加 {f2:1}；切回动态 = 恢复 6 个官方默认组件槽（槽18 特效 + 空槽 1/3/6/14/19）+ 删标记。实体联动：引用定义的场景实体同步切换。

#### 适用边界

适用于编辑器元件类型切换与 CLI assets:prefabs convert；纯静态类型（木质花圃）保留组件槽不适用本规律；root46 语义未闭合 CLI 不写。

<!-- CLAIM:END clm_D68D1E5F559964FD1512BB0E6A -->

<!-- CLAIM:START clm_01E3088B1E55DAB3E5FCC7103D -->

### 定义/页面模型/实体概念语义

定义（root4）= 元件本体；元件页面模型（root8）= 可视化编辑辅助（不渲染到场景，删除后定义仍在）；场景实体（root5）= 引用定义（官方 resID 或本地定义均可）。UI 静态元件分类 ≠ wire 无组件（纯静态类型保留组件槽）。定义 ID 分配只查 root4 空间，可与 root5/root8 ID 重叠。

#### 适用边界

用户 2026-08-20 概念澄清；解释 create --static（root8 模型可见非定义）、转定义（删模型+建定义）、实体引用等观察。

<!-- CLAIM:END clm_01E3088B1E55DAB3E5FCC7103D -->

<!-- CLAIM:START clm_0DED65DA905D38D1DBD032807B -->

### 删除自定义动态元件需四层整体移除

删除一个动态定制元件需同时整体移除四层：root4 定义记录、root8 实例记录、root27 该元件的 definition-side aux、root6 未分类页签组(type6)里的 {100, 定义ID} 登记条目。四层都是整体移除（非占位，区别于删除单个装饰物的 aux 空模型 10005018 占位）。动态元件的装饰物仅 definition-side 一组（实例/实体 f501 直接引用 definition-side aux，无独立 instance-side 副本），删 aux = 全部装饰物。遗漏任一层会产生孤儿记录（元件库仍显示 / 装饰物无宿主）。删除前须确认无 root5 实体引用该定义（有则悬空）、无节点图 createPrefab 用时定义 ID。

#### 适用边界

2026-08-23 魔方3x3(1073741899) 删除 26 个老版本块脚本实证 + assets:resources/inspect/export CLI 回读验证（删除定义26/实例26/装饰物54/登记26，回读残留0）；适用于本地自定义动态元件；官方引用式实例(f2={resID,1})无本地定义与装饰物的删除路径未验证。

<!-- CLAIM:END clm_0DED65DA905D38D1DBD032807B -->

<!-- CLAIM:START clm_EBA810B533A0A59E26B69BAC3D -->

### 装饰物颜色不透明度由 f3 Alpha 通道决定，f4 是冗余 float 百分比

装饰物 aux 材质槽 f5{t=22}.f32 的 f3 = 0xAARRGGBB，最高字节 A = 不透明度（0xFF=100% 不透明，0x02≈1%，几乎全透明、透出底层深灰块体 → 视觉贴片不是纯色）。f4 是 wire5 fixed32 承载的 IEEE754 float32，语义 = 不透明度百分比 A/255×100，与 f3 的 A 冗余一致：A=0xFF → f4=100.0(1120403456)，A=0x02 → f4=0.7843(1061734601)。修复须改 f3 的 A 通道（编辑器 UI 的 不透明度 即 A 通道），单独改 f4 不影响游戏渲染（f4 只是冗余存储）。排查贴片 颜色不对/不是纯色 先看 f3 最高字节 A，不要只比对 f3 的 RGB 低 24 位或 f4。

#### 适用边界

2026-08-23 魔方3x3 F 面心块贴片（星枢3x3块_v2_F，装饰物 1073741987）用户手动修正实证（f3 0x0235C96F→0xFF35C96F 修复）；f4=A/255×100 由两个样本（A=0x02/0xFF）闭合，中间 A 值的线性关系未穷举验证。

<!-- CLAIM:END clm_EBA810B533A0A59E26B69BAC3D -->

<!-- CLAIM:START clm_0ED0606E558B0290223FFC96E7 -->

### 元件页面模型 position 与 createPrefab 世界坐标是两套独立坐标系

root8 实例（页面模型）的 transform position 是编辑器/元件面板展示坐标，惯例以原点为中心（块间距 1、底部离地约 0.22）；节点图运行时 createPrefab(x,y,z) 用另一套世界坐标（3x3 魔方块中心在 2/3/4，魔方中心 3）。改 root8 实例 position 只影响编辑器/元件面板显示，不影响游戏运行时（游戏块由 createPrefab 动态生成实体，坐标来自节点图参数而非 root8 实例）。两套坐标按项目约定换算（本次魔方：root8 x/z = createPrefab x/z − 3，root8 y = createPrefab y − 1.3，使底部 D 层 y=0.7、块底离地约 0.22）。

#### 适用边界

2026-08-23 魔方3x3(1073741899) 26 个 v2 块实例位置改写实证 + export 回读验证（26/26 位置匹配）；两套坐标相互独立是通用结论，但具体换算差值（−3/−1.3）是本项目约定，不跨项目通用。

<!-- CLAIM:END clm_0ED0606E558B0290223FFC96E7 -->
