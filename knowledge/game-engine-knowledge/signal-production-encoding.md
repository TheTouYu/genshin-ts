...
<!-- CLAIM:START clm_B57B95C94FC41F8DFDAA236468 -->

### 同名图 GIA 导入语义：编辑器不合并，自动追加 _1 后缀创建新图，原图不变

编辑器导入同名节点图 GIA 时**不合并**：自动改名（追加 _1 后缀）并创建新图，原图保持不变。真实证据：consume-str-replay-v1.gia（SHA-256 ea369ae2e3a1e592828126986eff43c50b2658a27127225d3928d509eed849af）由用户确认编辑器导入成功（'测试完美通过'）；落盘结构与重放候选逐节点一致（含 connect2=3 例外），仅图名差异——同名图导入后新图 id=1073741847（16 节点），原图 1073741842 不变。

#### 适用边界

证据=用户编辑器导入确认（consume-str-replay-v1.gia）+ 落盘逐节点比对（重放候选 vs 编辑器保存）；适用于当前编辑器版本，不证明其他版本；仅覆盖同名图导入这一语义，其他导入冲突场景未验证

<!-- CLAIM:END clm_B57B95C94FC41F8DFDAA236468 -->
