...
<!-- CLAIM:START clm_4F59DF5C7CB9EEB3B39BDA8E0E -->

### 第四轮基础运动器与元件分类生产闭环的验证范围

第四轮在 mapId=1073741849 上创建分类‘运动器’，自动分配分类 ID 6，并将基础运动器定义 1077936175、1077936176 互斥归类；类型码 4 的默认基础运动器快照（后经 2026-08-13 差分修正；原记录的 101 bytes 槽即 type 18=模板自带组件槽，非基础运动器）在定义/实例对 1077936175/1077936177 与 1077936176/1077936178 上均为 101 bytes 且双侧逐字节一致。受限写回前 SHA-256 为 d30d6935c1c32bd00ec0327bcd85b4a7dab9123d3a3a57246f5ec447b836c8ca，工具写后为 2a1be1776ae8b582ccf567b509ee410b1976a0f262e58cb4a341c7fb14e6e36a；用户游戏测试通过，编辑器回存后地图 SHA-256 为 7c5006e72872001c7c0a1b283001b56a3aceb9ebedc349d3214bdecda52a7cb4、大小 268124 bytes，只读复扫再次确认分类 ID/两个成员、默认未分类互斥和两对 101-byte 双侧快照。

#### 适用边界

仅覆盖该 mapId 的上述地图状态、基础运动器 default 快照、分类创建 max+1 的当前真实结构和 kind=100 自定义定义互斥迁移；不推广到其它地图、组件类型、细粒度运动/特效参数、任意分类结构或 ID 分配规则。自动回归、写回、回存复扫和用户游戏验证是不同证据层。用户额外保存到‘基础元件’的 1077936177..1077936179 不属于本 Claim 的待调查范围。

<!-- CLAIM:END clm_4F59DF5C7CB9EEB3B39BDA8E0E -->
