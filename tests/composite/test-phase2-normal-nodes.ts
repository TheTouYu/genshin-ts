...
const branchWithPrint = g.defineComposite('分叉+打印叶子', {
  ...
  build(_inputs: any, f: any) {
    ...
    f.branchExec(0, { id: 0, type: 'exec', nodeType: 'print_string', args: [new str('分支B')] })
    return {}
  }
})
...
const loopWithPrint = g.defineComposite('循环+打印叶子', {
  ...
  build(inputs: any, f: any) {
    ...
    f.branchExec(0, { id: 0, type: 'exec', nodeType: 'print_string', args: [new str('循环体')] })  // OutFlow[0] = 循环体
    f.branchExec(1, { id: 0, type: 'exec', nodeType: 'print_string', args: [new str('完成')] })    // OutFlow[1] = 循环完成
    return {}
  }
})
...
const singlePrintLeaf = g.defineComposite('单打印出口', {
  ...
  outputs: {},
  build(_inputs: any, f: any) {
    f.registerExecNode('print_string', [new str('直接打印')])
    f.leaf(0)
    return {}
  }
})
...
g.server({ name: 'main', graphId: 1073741912 })
  .on('whenEntityIsCreated', (_e: any, f: any) => {
    // P2-S1: 分叉 + 打印叶子
    const r1 = f.callComposite(branchWithPrint, {})
    f.connectOutFlow(r1, 0, () => { f.printString('分支A-下游') })
    f.connectOutFlow(r1, 1, () => { f.printString('分支B-下游') })

    // P2-S2: 循环 + 打印叶子
    const r2 = f.callComposite(loopWithPrint, { start: new int(1n), end: new int(5n) })
    f.connectOutFlow(r2, 0, () => { f.printString('循环体-下游') })
    f.connectOutFlow(r2, 1, () => { f.printString('循环完成-下游') })

    // P2-S3: 条件分支
    const r3 = f.callComposite(condBranchWithInput, { '条件': new bool(true) })
    f.connectOutFlow(r3, 0, () => { f.printString('条件为真') })
    f.connectOutFlow(r3, 1, () => { f.printString('条件为假') })
...
for (const a of accs) {
  ...
  if (!d) continue
  console.log(`\n  ${d.name}:`)
  console.log(`    inflows=${d.inflows?.length} outflows=${d.outflows?.length} inputs=${d.inputs?.length} outputs=${d.outputs?.length}`)
...
for (const a of accs) {
  ...
  const defAcc = accs.find((aa: any) =>
    aa.which === 12 && aa.id?.id === a.id?.id - 10000 + a.id?.id // 不准确，换方法
  )
  // 通过 graphId 匹配
  let defName = ''
  ...
  for (const n of ig.nodes) {
    ...
    const pinTypes = n.pins?.map((p: any) => {
      const k = kname[p.i1?.kind] ?? '?'
      return `${k}:${p.i1?.index}`
    }).join(', ')
    const ofCount = n.pins?.filter((p: any) => p.i1?.kind === 2).length ?? 0
    console.log(`    n[${n.nodeIndex}] kind=${kind} nodeId=${nid} pins(${n.pins?.length}) [${pinTypes}]${ofCount > 1 ? ' ★MULTI-OF' : ''}`)
  }
...
const s1Impl = accs.find((a: any) => {
  ...
  return cps.length === 3 && cps.some((cp: any) => cp.outerPin?.kind === 2 && cp.outerPin?.index === 1)
})
if (s1Impl) {
  const ig = s1Impl.graph.inner.graph
  // 应该有 entry double_branch（OutFlow:0 分叉） + 2 个 print_string 叶子
  const entryNode = ig.nodes.find((n: any) => n.nodeIndex === 1)
...
compositeNodes.forEach((n: any) => {
  ...
  console.log(`  node[${n.nodeIndex}] (nodeId=${n.genericId?.nodeId}) ${ofPins.length} OutFlow pins`)
})
