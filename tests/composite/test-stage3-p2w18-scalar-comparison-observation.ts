// @ts-nocheck
/**
 * P2-W18: verify shared scalar comparison identity through executable data/control-flow graphs.
 *
 ...
 *
 * Run:
 *   npx tsx tests/composite/test-stage3-p2w18-scalar-comparison-observation.ts /tmp/P2W18-legacy.gia
 *   GSTS_STAGE3_VENDOR_IMPL_GRAPH=1 \
 *     npx tsx tests/composite/test-stage3-p2w18-scalar-comparison-observation.ts /tmp/P2W18-vendor.gia
 */
...
const OUTPUT_PATH = process.argv[2] ?? '/tmp/P2W18-scalar-comparison.gia'
const GRAPH_ID = 1073742418
const COMPOSITE_NAME = 'P2W18_ScalarComparisonFlow_GSTS'
...
g.server({ name: 'P2W18-scalar-comparison-flow', id: GRAPH_ID }).on(
  'whenEntityIsCreated',
  (_event: any, f: any) => {
...
const docs = buildServerGraphRegistriesIRDocuments({ defaultName: 'P2W18-scalar-comparison-flow' })
const bytes = irToGia(docs.at(-1), {
  graphId: GRAPH_ID,
  name: 'P2W18-scalar-comparison-flow',
...
console.log(
  `PASS P2-W18 ${USE_VENDOR_IMPL_GRAPH ? 'vendor-Graph' : 'legacy'} executable shared-resolution: ${OUTPUT_PATH}`
)
console.log('PENDING EDITOR REVIEW: control flow reaches Print String through comparison data paths')
