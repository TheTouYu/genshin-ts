...
const OUTPUT_PATH = process.argv[2] ?? `${EXPORT_DIR}/P2W11-nested-capture-vendor-graph-candidate.gia`
...
const docs = buildServerGraphRegistriesIRDocuments({ defaultName: 'P2W11-nested-capture-vendor-graph' })
...
assert.equal(captureRoute.innerPin?.index, 0, 'nested capture route must target child logical input index 0')
assert.equal(
...
console.log(`PASS P2-W11 ${USE_VENDOR_IMPL_GRAPH ? 'vendor-Graph candidate' : 'legacy baseline'}: ${OUTPUT_PATH}`)
