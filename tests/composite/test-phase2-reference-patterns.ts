...
const mixedLeaves = g.defineComposite('混合叶子', {
  ...
  build(_inputs: any, f: any) {
    ...
    const db2 = f.branchExec(0, { id: 0, type: 'exec', nodeType: 'double_branch', args: [] })
    const pC = f.branchExec(0, {
      id: 0,
      type: 'exec',
      nodeType: 'print_string',
      args: [new str('出口C')]
...
const mixedLeavesNo = g.defineComposite('混合叶子-否', {
  ...
  build(_inputs: any, f: any) {
    ...
    const pX = f.branchExec(1, {
      id: 0,
      type: 'exec',
      nodeType: 'print_string',
      args: [new str('出口X')]
    })
    const pY = f.branchExec(1, {
      id: 0,
      type: 'exec',
      nodeType: 'print_string',
      args: [new str('出口Y')]
...
const multiOutData = g.defineComposite('多出口+数据', {
  ...
  build(inputs: any, f: any) {
    ...
    // 执行分叉: 4 出口
    f.registerExecNode('double_branch', [new bool(true)])
    const p1 = f.branchExec(0, {
      id: 0,
      type: 'exec',
      nodeType: 'print_string',
      args: [new str('路径1')]
    })
    const p2 = f.branchExec(0, {
      id: 0,
      type: 'exec',
      nodeType: 'print_string',
      args: [new str('路径2')]
    })
    const p3 = f.branchExec(0, {
      id: 0,
      type: 'exec',
      nodeType: 'print_string',
      args: [new str('路径3')]
    })
    const p4 = f.branchExec(0, {
      id: 0,
      type: 'exec',
      nodeType: 'print_string',
      args: [new str('路径4')]
...
g.server({ name: 'main', graphId: 1073741913 }).on('whenEntityIsCreated', (_e: any, f: any) => {
  // P1: 纯数据
  ...
  f.connectOutFlow(d2, 0, () => {
    f.printString('条件为真-路径')
  })
  f.connectOutFlow(d2, 1, () => {
    f.printString('条件为假-路径')
  })

  // P3a: 混合叶子-是 (显式 true)
  const d3a = f.callComposite(mixedLeaves, {})
  f.connectOutFlow(d3a, 0, () => {
    f.printString('出口A下游')
  })
  f.connectOutFlow(d3a, 1, () => {
    f.printString('出口B下游')
  })
  f.connectOutFlow(d3a, 2, () => {
    f.printString('出口C下游')
  })

  // P3b: 混合叶子-否 (无参，默认 false)
  const d3b = f.callComposite(mixedLeavesNo, {})
  f.connectOutFlow(d3b, 0, () => {
    f.printString('出口X下游')
  })
  f.connectOutFlow(d3b, 1, () => {
    f.printString('出口Y下游')
  })
  ...
  f.connectOutFlow(d5, 0, () => {
    f.printString('路径1-下游')
  })
  f.connectOutFlow(d5, 1, () => {
    f.printString('路径2-下游')
  })
})
...
for (const a of accs) {
  ...
  console.log(
    `  ${d.name}: in=${d.inflows?.length} out=${d.outflows?.length} inp=${d.inputs?.length} outp=${d.outputs?.length}`
  )
}
...
for (const a of accs) {
  ...
  console.log(
    `  ${defName}: ${ig.nodes?.length} nodes, compositePins: InFlow/OutFlow=${ofCount} InParam=${ipCount} OutParam=${opCount}`
  )
  ...
  for (const n of ig.nodes) {
    ...
    const pinSummary =
      n.pins?.map((p: any) => `${kname[p.i1?.kind]}:${p.i1?.index}`).join(' ') ?? '(none)'
...
// P2 特定验证: 条件输入走 compositePin 路由，不在 double_branch 上生成物理 InParam pin
...
if (p2Impl) {
  const dbNode = p2Impl.graph.inner.graph.nodes.find(
    (n: any) => (n.genericId?.nodeId ?? n.concreteId?.nodeId) === 2
  )
  if (dbNode) {
    const hasInParam = dbNode.pins?.some((p: any) => p.i1?.kind === 3)
    check(!hasInParam, `P2: double_branch 无物理 InParam (条件输入由 compositePin 路由)`)
    const hasConditionPin = p2Impl.graph.inner.graph.compositePins?.some(
      (cp: any) => cp.outerPin?.kind === 3 && cp.innerNodeId === 2
    )
    check(hasConditionPin, `P2: compositePins 包含条件输入路由`)
  }
}
...
if (p5Impl) {
  const ofPins =
    p5Impl.graph.inner.graph.compositePins?.filter((cp: any) => cp.outerPin?.kind === 2) ?? []
  const opPins =
    p5Impl.graph.inner.graph.compositePins?.filter((cp: any) => cp.outerPin?.kind === 4) ?? []
...
compCalls.forEach((n: any) => {
  ...
  console.log(
    `  node[${n.nodeIndex}] nodeId=${n.genericId?.nodeId}: ${ofCount} OF, ${ipCount} IP, ${opCount} OP`
  )
})
