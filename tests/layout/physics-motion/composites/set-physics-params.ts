...
export const setPhysicsParams = g.defineComposite('设置物理参数', {
  ...
  build(args, f) {
    ...
    const gravity = asRuntimeValue(
      f.getCustomVariable(targetEntity, PHYSICS_CUSTOM_VARIABLES.gravity).asType('float')
    )
    const stiffness = asRuntimeValue(
      f.getCustomVariable(targetEntity, PHYSICS_CUSTOM_VARIABLES.stiffness).asType('float')
    )
    const inverseInertia = asRuntimeValue(
      f.getCustomVariable(targetEntity, PHYSICS_CUSTOM_VARIABLES.inverseInertia).asType('float')
    )
    const damping = asRuntimeValue(
      f.getCustomVariable(targetEntity, PHYSICS_CUSTOM_VARIABLES.damping).asType('float')
    )
    const radius = asRuntimeValue(
      f.getCustomVariable(targetEntity, PHYSICS_CUSTOM_VARIABLES.radius).asType('float')
    )
    const friction = asRuntimeValue(
      f.getCustomVariable(targetEntity, PHYSICS_CUSTOM_VARIABLES.friction).asType('float')
    )
    const mass = asRuntimeValue(
      f.getCustomVariable(targetEntity, PHYSICS_CUSTOM_VARIABLES.mass).asType('float')
    )
    ...
    const visualEntityGuid = f
      ...
      .asType('guid')
    const angularFriction = asRuntimeValue(
      f.getCustomVariable(targetEntity, PHYSICS_CUSTOM_VARIABLES.angularFriction).asType('float')
    )
    const gravityForce = asRuntimeValue(
      f.getCustomVariable(targetEntity, PHYSICS_CUSTOM_VARIABLES.gravityForce).asType('float')
    )
    ...
    const setHalfGravityDeltaSquared = f.node('set_node_graph_variable', [
      ...
      asRuntimeValue(halfGravityDeltaSquared.result),
      new boolValue(false)
    ])
